...
"""Independently replay the fixed local deposited-pH recovery-v6 output.
...
CHECKER_RELATIVE = Path(
    "gpuopt/candidates/check_solution_state_openmm86_deposited_ph_catalog_recovery_v6.py"
)
PLAN_RELATIVE = Path(
    "gpuopt/preunblind/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v6_plan.json"
)
...
ARTIFACT_RELATIVE = Path(
    ".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_api_v6_recovery"
)
...
CONTRACT = "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v6"
PLAN_CONTRACT = "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v6_plan"
API_BASE = "https://api.bmrb.io/v2"
APPLICATION_HEADER = "AtypEmu openmm86-deposited-ph-catalog-recovery-v6"
ENTITY_COUNT = 135
...
# Copied from the raw-byte-bound recovery-v6 plan.  These are fixed local
...
V3_FAILED_LAUNCH_INTENT_RELATIVE = Path(
    ".auto/staging/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v3_launch_intent.failed_run126.json"
)
...
V3_FAILED_LAUNCH_INTENT = {
    ...
    # This is deliberately the failed launch's erroneous full commit, not a
    # corrected v4 provenance value.
    "git_commit": "44d636dbf5374e4596290e05015ab0bd145c87d5",
    "plan": {
    ...
    "output": V3_OUTPUT_RELATIVE.as_posix(),
    "target_values_read": False,
    "target_atom_identities_read": False,
    "source_scores_read": False,
    "science_executed": False,
...
V3_FAILED_LAUNCH_INTENT_BINDING = {
    "path": V3_FAILED_LAUNCH_INTENT_RELATIVE.as_posix(),
    "sha256": "c75986bea0ce11684fec2d0aae6aa594ec65527a9814cd89525083f1bd9d7a54",
    "exact_fields": V3_FAILED_LAUNCH_INTENT,
}
RUN122_LOG_BINDING = {
...
RUN126_LOG_BINDING = {
    "path": LOG_RELATIVE.as_posix(),
    # The digest includes the terminating LF; reject equivalent JSON without it.
    "raw_line_sha256": "0fe712c190d65653ff0521f3bb8c3518fef96296c80354be4a1b1c9c8cfdd15a",
    "semantic_subset": {
        "run": 126,
        "commit": "44d636d",
        "metric": 0,
        "status": "crash",
        "description": "Run 126: recovery-v3 launch failed closed on incorrect full commit binding",
        "asi_required_fields": ["error", "failure_stage", "observed_controls", "rollback_reason"],
        "observed_controls": "all leaves are false or integer zero",
    },
}
LEGACY_PRE_OUTPUT_GATES = {
    "actual_v1_failure_tree": V1_FAILURE_BINDING,
    "active_v2_launch_intent": V2_LAUNCH_INTENT_BINDING,
    "failed_v3_launch_intent": V3_FAILED_LAUNCH_INTENT_BINDING,
    "run_122_log_record": RUN122_LOG_BINDING,
    "run_126_log_record": RUN126_LOG_BINDING,
    "v2_output_must_be_absent": V2_OUTPUT_RELATIVE.as_posix(),
    "v3_output_must_be_absent": V3_OUTPUT_RELATIVE.as_posix(),
}
LEGACY_RECOVERY_EVIDENCE = {
    ...
    "run_122_log_record": RUN122_LOG_BINDING,
    "run_126_log_record": RUN126_LOG_BINDING,
    "v2_output_absent_before_v4_creation": V2_OUTPUT_RELATIVE.as_posix(),
    "v3_output_absent_before_v4_creation": V3_OUTPUT_RELATIVE.as_posix(),
}

# Recovery-v6 admits only the sealed first-request v5 failure.  The complete
# consumed-launch hash was supplied; the remaining task evidence was supplied
# only as full SHA-256 digests and is represented honestly as such.
V5_CONSUMED_INTENT_RELATIVE = Path(".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_launch_intent_v2.consumed.json")
V5_ACTIVE_INTENT_RELATIVE = Path(".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_launch_intent_v2.json")
V5_EXECUTION_RECEIPT_RELATIVE = Path(".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_execution_receipt_v2.json")
V5_FALLBACK_RECEIPT_RELATIVE = Path(".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_execution_receipt_v2_fallback.json")
V5_FAILURE_RELATIVE = Path(".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_api_v5_recovery")
V5_SUCCESS_RECEIPT_RELATIVE = V5_FAILURE_RELATIVE / "receipt.json"
V5_FAILURE_REQUIRED_FIELDS = {
    ...
    "contract": "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v5",
    "new_response_count_written": 0,
    "new_response_manifest": [],
    **CLOSED_FLAGS,
}
V5_CONSUMED_INTENT_BINDING = {"path": V5_CONSUMED_INTENT_RELATIVE.as_posix(), "sha256": "2d767f8e825ebd3f89e4d2c8e68d4f88e24306650bea2847c7c69a382c3d0c83"}
V5_FAILURE_BINDING = {
    "failure_directory": V5_FAILURE_RELATIVE.as_posix(),
    "failure_inventory": {"regular_files": ["failure_receipt.json", "started_at_utc.txt"], "directories": [{"path": "raw_api_responses", "inventory": []}]},
    "failure_receipt": {"path": "failure_receipt.json", "sha256": "dc27b82d7970eededd779c8ffa891e00cf346ee6b5ffbca23efe63ebbf136a1f", "required_fields": V5_FAILURE_REQUIRED_FIELDS},
    "started_at_utc": {"path": "started_at_utc.txt", "sha256": "a4674722b9e6e5d19210f726b69429c9eca325dc207bc574158f7ccf084a524e"},
}
V5_EXECUTION_RECEIPT_BINDING = {"path": V5_EXECUTION_RECEIPT_RELATIVE.as_posix(), "sha256": "99cc7c45d8749d08ab3c4a4005a9634ce0fc7884e0fd2b77ce2b2b10369d3f78"}
RUN136_LOG_BINDING = {"path": LOG_RELATIVE.as_posix(), "raw_line_sha256": "954b294033b90eb75d0106790ccc117e8514a8bb9f8674ee6d2c35acdaea252d", "semantic_subset": {"run": 136, "status": "crash"}}
PRE_OUTPUT_GATES = {
    **LEGACY_PRE_OUTPUT_GATES,
    "consumed_v5_launch_intent": V5_CONSUMED_INTENT_BINDING,
    "actual_v5_failure_tree": V5_FAILURE_BINDING,
    "v5_execution_receipt": V5_EXECUTION_RECEIPT_BINDING,
    "run_136_log_record": RUN136_LOG_BINDING,
    "active_v5_launch_intent_must_be_absent": V5_ACTIVE_INTENT_RELATIVE.as_posix(),
    "v5_success_receipt_must_be_absent": V5_SUCCESS_RECEIPT_RELATIVE.as_posix(),
    "v5_fallback_execution_receipt_must_be_absent": V5_FALLBACK_RECEIPT_RELATIVE.as_posix(),
}
RECOVERY_EVIDENCE = {
    "legacy_recovery_evidence": LEGACY_RECOVERY_EVIDENCE,
    "consumed_v5_launch_intent": V5_CONSUMED_INTENT_BINDING,
    "v5_failure_tree": V5_FAILURE_BINDING,
    "v5_execution_receipt": V5_EXECUTION_RECEIPT_BINDING,
    "run_136_log_record": RUN136_LOG_BINDING,
}
...
ALLOWED_TAGS = {
    ...
    "Experiment": frozenset(
        (
            ...
            "Chromatographic_column_ID", "Chromatographic_column_label",
            "Chromatographic_system_label", "Details",
...
# Exact archive-wide, non-target Experiment metadata observation for v6.
OBSERVED_EXPERIMENT_TAG_UNION = frozenset(ALLOWED_TAGS["Experiment"])
assert len(OBSERVED_EXPERIMENT_TAG_UNION) == 46
...
def _load_plan_and_producer(root: Path) -> tuple[dict[str, Any], str, str]:
    plan_head = _committed_exact(root, PLAN_RELATIVE, PLAN_RAW_SHA256, "recovery-v6 plan")
    producer_head = _committed_exact(
        root, PRODUCER_RELATIVE, PRODUCER_RAW_SHA256, "recovery-v6 producer"
    )
    plan = _decode(
        _read_regular(root, PLAN_RELATIVE, "recovery-v6 plan", MAX_SOURCE_BYTES),
        "recovery-v6 plan",
    )
    if not isinstance(plan, dict):
        raise ValueError("recovery-v6 plan root is not an object")
    ...
    if (
        ...
        or not _exact(plan.get("recovery_of"), RECOVERY_EVIDENCE)
        or not _exact(plan.get("pre_output_gates"), PRE_OUTPUT_GATES)
    ):
        raise ValueError("recovery-v6 plan semantic binding drifted")
...
def _validate_v1_failure(root: Path, binding: Any = V1_FAILURE_BINDING) -> dict[str, Any]:
    ...
    if not _exact(binding, V1_FAILURE_BINDING):
        raise ValueError("recovery-v6 v1 failure binding drifted")
...
def _validate_failed_v3_launch_intent(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    """Bind the archived failed v3 launch, including its erroneous commit."""
    fixed = binding is None
    binding = V3_FAILED_LAUNCH_INTENT_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict)
        and set(binding) == {"path", "sha256", "exact_fields"}
        and binding.get("path") == V3_FAILED_LAUNCH_INTENT_RELATIVE.as_posix()
        and isinstance(binding.get("sha256"), str)
        and SHA256_RE.fullmatch(binding["sha256"])
        and _exact(binding.get("exact_fields"), V3_FAILED_LAUNCH_INTENT)
        and (not fixed or _exact(binding, V3_FAILED_LAUNCH_INTENT_BINDING))
    ):
        raise ValueError("failed recovery-v3 launch-intent binding drifted")
    raw = _read_regular(
        root,
        V3_FAILED_LAUNCH_INTENT_RELATIVE,
        "failed recovery-v3 launch intent",
        MAX_SOURCE_BYTES,
    )
    if _sha256(raw) != binding["sha256"]:
        raise ValueError("failed recovery-v3 launch-intent raw SHA-256 drifted")
    if not _exact(_decode(raw, "failed recovery-v3 launch intent"), binding["exact_fields"]):
        raise ValueError("failed recovery-v3 launch-intent schema or values drifted")
    return V3_FAILED_LAUNCH_INTENT_BINDING if fixed else binding
...
def _validate_run126_log(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    """Require the one Run 126 record whose bound byte includes its final LF."""
    fixed = binding is None
    binding = RUN126_LOG_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict)
        and set(binding) == {"path", "raw_line_sha256", "semantic_subset"}
        and binding.get("path") == LOG_RELATIVE.as_posix()
        and isinstance(binding.get("raw_line_sha256"), str)
        and SHA256_RE.fullmatch(binding["raw_line_sha256"])
        and _exact(binding.get("semantic_subset"), RUN126_LOG_BINDING["semantic_subset"])
        and (not fixed or _exact(binding, RUN126_LOG_BINDING))
    ):
        raise ValueError("Run 126 log binding drifted")
    raw_log = _read_regular(root, LOG_RELATIVE, "append-only operational log", MAX_LOG_BYTES)
    lines = raw_log.splitlines(keepends=True)
    if len(lines) > MAX_LOG_LINES:
        raise ValueError("append-only operational log exceeds the line limit")
    matches = [
        ...
        if line.endswith(b"\n") and _sha256(line) == binding["raw_line_sha256"]
    ]
    if len(matches) != 1:
        raise ValueError("Run 126 exact newline-terminated raw log line is absent or duplicated")
    record = _decode(matches[0], "Run 126 log line")
    semantic = binding["semantic_subset"]
    if not isinstance(record, dict) or not (
        type(record.get("run")) is int and record["run"] == semantic["run"]
        and record.get("commit") == semantic["commit"]
        ...
        raise ValueError("Run 126 log-line semantic subset drifted")
    required = set(semantic["asi_required_fields"])
    matches_asi: list[dict[str, Any]] = []

    def visit(value: Any) -> None:
        if isinstance(value, dict):
            if required.issubset(value):
                matches_asi.append(value)
            for child in value.values():
                visit(child)
        elif isinstance(value, list):
            for child in value:
    ...
    visit(record)
    if len(matches_asi) != 1:
        raise ValueError("Run 126 ASI schema is absent or ambiguous")
    asi = matches_asi[0]
    if any(
        not isinstance(asi[name], str) or not asi[name]
        for name in ("error", "failure_stage", "rollback_reason")
    ):
        raise ValueError("Run 126 ASI text fields drifted")
    if not isinstance(asi["observed_controls"], dict) or not _closed_control_tree(
        asi["observed_controls"]
    ):
        raise ValueError("Run 126 ASI observed controls are not all closed")
    return RUN126_LOG_BINDING if fixed else binding


def _require_output_absent(root: Path, relative: Path, label: str) -> None:
    """Reject any predecessor output, including a symlinked path component."""
    current = root.resolve(strict=True)
    pieces = _safe_relative(relative.as_posix(), "%s output" % label).split("/")
    for index, piece in enumerate(pieces):
        ...
        try:
            ...
            raise ValueError("cannot inspect recovery-%s output absence" % label) from error
        if stat.S_ISLNK(details.st_mode):
            raise ValueError("recovery-%s output absence path is indirect" % label)
        if index == len(pieces) - 1:
            raise ValueError("recovery-%s output exists; in-place retry is forbidden" % label)
        if not stat.S_ISDIR(details.st_mode):
            raise ValueError("recovery-%s output parent is not a directory" % label)


def _require_v2_output_absent(root: Path) -> None:
    _require_output_absent(root, V2_OUTPUT_RELATIVE, "v2")


def _require_v3_output_absent(root: Path) -> None:
    _require_output_absent(root, V3_OUTPUT_RELATIVE, "v3")


def _validate_legacy_pre_output_gates(
    root: Path, gates: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    """Validate every sealed v1/v2/v3 admission fact before output reads."""
    fixed = gates is None
    gates = LEGACY_PRE_OUTPUT_GATES if gates is None else gates
    if not (
        isinstance(gates, dict)
        and set(gates) == set(LEGACY_PRE_OUTPUT_GATES)
        and _exact(gates.get("actual_v1_failure_tree"), V1_FAILURE_BINDING)
        and gates.get("v2_output_must_be_absent") == V2_OUTPUT_RELATIVE.as_posix()
        and gates.get("v3_output_must_be_absent") == V3_OUTPUT_RELATIVE.as_posix()
        and (not fixed or _exact(gates, LEGACY_PRE_OUTPUT_GATES))
    ):
        raise ValueError("recovery-v6 pre-output gate schema or fixed paths drifted")
    evidence = {
        "v1_failure_tree": _validate_v1_failure(root),
        "v2_launch_intent": _validate_v2_launch_intent(root, gates["active_v2_launch_intent"]),
        "failed_v3_launch_intent": _validate_failed_v3_launch_intent(
            root, gates["failed_v3_launch_intent"]
        ),
        "run_122_log_record": _validate_run122_log(root, gates["run_122_log_record"]),
        "run_126_log_record": _validate_run126_log(root, gates["run_126_log_record"]),
        "v2_output_absent_before_v4_creation": V2_OUTPUT_RELATIVE.as_posix(),
        "v3_output_absent_before_v4_creation": V3_OUTPUT_RELATIVE.as_posix(),
    }
    _require_v2_output_absent(root)
    _require_v3_output_absent(root)
    if fixed and not _exact(evidence, LEGACY_RECOVERY_EVIDENCE):
        raise ValueError("recovery-v6 pre-output evidence drifted")
    return LEGACY_RECOVERY_EVIDENCE if fixed else evidence


def _required(value: Any, expected: Any) -> bool:
    if type(value) is not type(expected):
        return False
    if isinstance(expected, dict):
        return all(name in value and _required(value[name], wanted) for name, wanted in expected.items())
    if isinstance(expected, list):
        return len(value) == len(expected) and all(
            _required(item, wanted) for item, wanted in zip(value, expected)
        )
    return value == expected


def _matches_hash_attestation(raw: bytes, spec: dict[str, Any], label: str) -> None:
    full = spec.get("sha256")
    digest = _sha256(raw)
    if not isinstance(full, str) or SHA256_RE.fullmatch(full) is None or digest != full:
        raise ValueError("%s raw SHA-256 drifted" % label)


def _v5_failure_directory(root: Path) -> Path:
    return _directory_path(root, V5_FAILURE_RELATIVE, "sealed v5 failure directory")


def _validate_v5_consumed_intent(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    fixed = binding is None
    binding = V5_CONSUMED_INTENT_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict) and set(binding) == {"path", "sha256"}
        and binding.get("path") == V5_CONSUMED_INTENT_RELATIVE.as_posix()
        and isinstance(binding.get("sha256"), str) and SHA256_RE.fullmatch(binding["sha256"])
        and (not fixed or _exact(binding, V5_CONSUMED_INTENT_BINDING))
    ):
        raise ValueError("consumed v4 launch-intent binding drifted")
    raw = _read_regular(root, V5_CONSUMED_INTENT_RELATIVE, "consumed v5 launch-v2", MAX_SOURCE_BYTES)
    if _sha256(raw) != binding["sha256"]:
        raise ValueError("consumed v4 launch intent raw SHA-256 drifted")
    return binding
...
def _validate_v5_failure(
    ...
    fixed = binding is None
    binding = V5_FAILURE_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict)
        and set(binding) == {"failure_directory", "failure_inventory", "failure_receipt", "started_at_utc"}
        and binding.get("failure_directory") == V5_FAILURE_RELATIVE.as_posix()
        and _required(binding.get("failure_receipt"), {
            "path": "failure_receipt.json", "required_fields": V5_FAILURE_REQUIRED_FIELDS,
        })
        and _required(binding.get("started_at_utc"), {"path": "started_at_utc.txt"})
        and (not fixed or _exact(binding, V5_FAILURE_BINDING))
    ):
        raise ValueError("sealed v4 failure-tree binding drifted")
    receipt_spec = binding["failure_receipt"]
    marker_spec = binding["started_at_utc"]
    if not (
        set(receipt_spec) == {"path", "sha256", "required_fields"}
        and set(marker_spec) == {"path", "sha256"}
    ):
        raise ValueError("sealed v5 failure-tree hash binding drifted")
    directory = _v5_failure_directory(root)
    inventory = binding["failure_inventory"]
    if not isinstance(inventory, dict) or inventory.get("regular_files") != [
        "failure_receipt.json", "started_at_utc.txt"
    ] or inventory.get("directories") != [{"path": "raw_api_responses", "inventory": []}]:
        raise ValueError("sealed v4 failure inventory binding drifted")
    try:
        with os.scandir(directory) as entries:
            files: set[str] = set()
            ...
            for entry in entries:
                if entry.is_symlink():
                    raise ValueError("sealed v4 failure inventory has a symlink")
                if entry.is_file(follow_symlinks=False):
                    files.add(entry.name)
                elif entry.is_dir(follow_symlinks=False):
                    directories.add(entry.name)
                else:
                    raise ValueError("sealed v4 failure inventory has an indirect entry")
    except OSError as error:
        raise ValueError("cannot inspect sealed v4 failure directory") from error
    if files != {"failure_receipt.json", "started_at_utc.txt"} or directories != {"raw_api_responses"}:
        raise ValueError("sealed v4 failure inventory drifted")
    try:
        with os.scandir(directory / "raw_api_responses") as entries:
            if any(True for _ in entries):
                raise ValueError("sealed v4 raw-response directory is not exactly empty")
    except OSError as error:
        raise ValueError("cannot inspect sealed v4 raw-response directory") from error
    receipt_raw = _read_regular(root, V5_FAILURE_RELATIVE / "failure_receipt.json", "sealed v5 failure receipt", MAX_JSON_BYTES)
    marker_raw = _read_regular(root, V5_FAILURE_RELATIVE / "started_at_utc.txt", "sealed v5 start marker", 1024)
    _matches_hash_attestation(receipt_raw, receipt_spec, "sealed v5 failure receipt")
    _matches_hash_attestation(marker_raw, marker_spec, "sealed v5 start marker")
    if not _required(_decode(receipt_raw, "sealed v5 failure receipt"), receipt_spec["required_fields"]):
        raise ValueError("sealed v5 failure receipt fields drifted")
    return binding


def _validate_run136_log(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    fixed = binding is None
    binding = RUN136_LOG_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict) and set(binding) == {"path", "raw_line_sha256", "semantic_subset"}
        and binding.get("path") == LOG_RELATIVE.as_posix()
        and isinstance(binding.get("raw_line_sha256"), str)
        and SHA256_RE.fullmatch(binding["raw_line_sha256"])
        and _exact(binding.get("semantic_subset"), {"run": 136, "status": "crash"})
        and (not fixed or _exact(binding, RUN136_LOG_BINDING))
    ):
        raise ValueError("Run 136 log binding drifted")
    raw_log = _read_regular(root, LOG_RELATIVE, "append-only operational log", MAX_LOG_BYTES)
    lines = raw_log.splitlines(keepends=True)
    if len(lines) > MAX_LOG_LINES:
        raise ValueError("append-only operational log exceeds the line limit")
    ...
    for line in lines:
        ...
        candidates = [line] if line == stripped else [line, stripped]
        hits = [item for item in candidates if item and _sha256(item) == binding["raw_line_sha256"]]
        if len(hits) > 1:
            raise ValueError("Run 131 raw log-line representation is ambiguous")
        matches.extend(hits)
    if len(matches) != 1:
        raise ValueError("Run 136 bound raw log line is absent or duplicated")
    record = _decode(matches[0].rstrip(b"\r\n"), "Run 136 log line")
    if not isinstance(record, dict) or type(record.get("run")) is not int or record.get("run") != 136 or record.get("status") != "crash":
        raise ValueError("Run 136 log-line semantic subset drifted")
    return binding


def _validate_v5_execution_receipt(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    fixed = binding is None
    binding = V5_EXECUTION_RECEIPT_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict) and set(binding) == {"path", "sha256"}
        and binding.get("path") == V5_EXECUTION_RECEIPT_RELATIVE.as_posix()
        and (not fixed or _exact(binding, V5_EXECUTION_RECEIPT_BINDING))
    ):
        raise ValueError("v5 execution-receipt binding drifted")
    raw = _read_regular(root, V5_EXECUTION_RECEIPT_RELATIVE, "v5 execution receipt", MAX_JSON_BYTES)
    _matches_hash_attestation(raw, binding, "v5 execution receipt")
    receipt = _decode(raw, "v5 execution receipt")
    if not isinstance(receipt, dict) or receipt.get("candidate_id") != "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5" or receipt.get("contract") != "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_execution_v2" or receipt.get("outcome") != "producer_failed" or receipt.get("closed_controls") != CLOSED_FLAGS:
        raise ValueError("v5 execution-receipt semantics drifted")
    return binding


def _require_v5_absent(root: Path, relative: Path, label: str) -> None:
    if os.path.lexists(root / relative):
        raise ValueError("%s exists; a separately named v6 recovery is required" % label)


def _validate_pre_output_gates(
    root: Path, gates: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    fixed = gates is None
    gates = PRE_OUTPUT_GATES if gates is None else gates
    if not (
        isinstance(gates, dict) and set(gates) == set(PRE_OUTPUT_GATES)
        and gates.get("active_v5_launch_intent_must_be_absent") == V5_ACTIVE_INTENT_RELATIVE.as_posix()
        and gates.get("v5_success_receipt_must_be_absent") == V5_SUCCESS_RECEIPT_RELATIVE.as_posix()
        and gates.get("v5_fallback_execution_receipt_must_be_absent") == V5_FALLBACK_RECEIPT_RELATIVE.as_posix()
        and (not fixed or _exact(gates, PRE_OUTPUT_GATES))
    ):
        raise ValueError("recovery-v6 pre-output gate schema drifted")
    legacy = _validate_legacy_pre_output_gates(root, {name: gates[name] for name in LEGACY_PRE_OUTPUT_GATES})
    evidence = {
        "legacy_recovery_evidence": legacy,
        "consumed_v5_launch_intent": _validate_v5_consumed_intent(root, gates["consumed_v5_launch_intent"]),
        "v5_failure_tree": _validate_v5_failure(root, gates["actual_v5_failure_tree"]),
        "v5_execution_receipt": _validate_v5_execution_receipt(root, gates["v5_execution_receipt"]),
        "run_136_log_record": _validate_run136_log(root, gates["run_136_log_record"]),
    }
    _require_v5_absent(root, V5_ACTIVE_INTENT_RELATIVE, "active v5 launch-v2 intent")
    _require_v5_absent(root, V5_SUCCESS_RECEIPT_RELATIVE, "v5 success receipt")
    _require_v5_absent(root, V5_FALLBACK_RECEIPT_RELATIVE, "v5 fallback execution receipt")
    if fixed and not _exact(evidence, RECOVERY_EVIDENCE):
        raise ValueError("recovery-v6 pre-output evidence drifted")
    return RECOVERY_EVIDENCE if fixed else evidence
...
def _rows(payload: bytes, entry_id: str, loop: str) -> list[dict[str, Any]]:
    ...
    if loop == NEW_LOOP:
        if not isinstance(entry, dict) or set(entry) != set(ASSIGNED_RESPONSE_KEYS):
            raise ValueError("assigned-list response keys are not exactly the three metadata keys")
        columns = [entry[key] for key in ASSIGNED_RESPONSE_KEYS]
        if not all(isinstance(column, list) for column in columns):
            raise ValueError("assigned-list response columns are not lists")
        if len({len(column) for column in columns}) != 1:
            raise ValueError("assigned-list response columns have unequal lengths")
        if not all(type(value) is str for column in columns for value in column):
            raise ValueError("assigned-list response values are not scalar strings")
        rows = [
            {"Entry_ID": values[0], "ID": values[1], "Sample_condition_list_ID": values[2]}
            for values in zip(*columns)
        ]
        if any(row["Entry_ID"] != entry_id for row in rows):
            raise ValueError("assigned-list response entry identity drifted")
        return rows
    if entry == {}:
        return []
...
def _load_prior(raw: bytes, roster: dict[str, dict[str, Any]]) -> dict[str, dict[str, bytes]]:
    ...
    observed_experiment_tags: set[str] = set()
    try:
        with zipfile.ZipFile(io.BytesIO(raw)) as archive:
            ...
            for entity_uid in sorted(roster):
                ...
                for loop in PRIOR_LOOPS:
                    ...
                    if loop == "Experiment":
                        decoded = _decode(payload, "sealed Experiment response")
                        entry = decoded[bmrb_id[3:]]
                        if entry != {}:
                            observed_experiment_tags.update(
                                tag for instance in entry["Experiment"] for tag in instance["tags"]
                            )
                    selected[loop] = payload
                result[entity_uid] = selected
        if frozenset(observed_experiment_tags) != OBSERVED_EXPERIMENT_TAG_UNION:
            raise ValueError("archive-wide observed Experiment tag union drifted")
...
def _requested_url(bmrb_id: str) -> str:
    if not isinstance(bmrb_id, str) or BMRB_RE.fullmatch(bmrb_id) is None:
        raise ValueError("noncanonical BMRB ID in requested URL")
    return "%s/entry/%s?%s" % (
        API_BASE,
        bmrb_id[3:],
        urllib.parse.urlencode([("tag", key) for key in ASSIGNED_RESPONSE_KEYS]),
    )
...
def _validate_receipt_static(
    ...
    plan_revision = _commit_has_bytes(
        root, plan.get("git_commit"), PLAN_RELATIVE, PLAN_RAW_SHA256, "recovery-v6 plan"
    )
    producer_revision = _commit_has_bytes(
        root, receipt.get("source_producer_git_commit"), PRODUCER_RELATIVE,
        PRODUCER_RAW_SHA256, "recovery-v6 producer",
    )
    if plan_revision != producer_revision:
        raise ValueError("receipt plan and producer commits differ")
    _require_ancestor(root, plan_revision, plan_head, "recovery-v6 plan")
    _require_ancestor(root, producer_revision, producer_head, "recovery-v6 producer")
...
def _validate_artifact_tree(root: Path, started: str, expected_files: set[str]) -> None:
    artifact = _directory_path(root, ARTIFACT_RELATIVE, "recovery-v6 output")
    try:
        with os.scandir(artifact) as entries:
            names = set()
            for entry in entries:
                if len(names) >= 3:
                    raise ValueError("recovery-v6 output inventory exceeds its bound")
                if entry.is_symlink():
                    raise ValueError("symlink in recovery-v6 output")
                names.add(entry.name)
    except OSError as error:
        raise ValueError("cannot inspect recovery-v6 output") from error
    if names != {RAW_DIRECTORY, "receipt.json", START_MARKER}:
        raise ValueError("recovery-v6 output inventory drifted")
...
def verify_artifact() -> tuple[int, str]:
    ...
    if not _exact(plan["pre_output_gates"], PRE_OUTPUT_GATES) or not _exact(
        plan["recovery_of"], RECOVERY_EVIDENCE
    ): 
        raise ValueError("recovery-v6 plan pre-output bindings drifted")

    receipt = _decode(
        _read_regular(root, RECEIPT_RELATIVE, "recovery-v6 receipt", MAX_JSON_BYTES),
        "recovery-v6 receipt",
    )
...
def _fixture(
    *,
    ...
    def wrapped(loop: str, tags: list[str], values: list[list[Any]]) -> bytes:
        if loop == NEW_LOOP:
            if tuple(tags) != ("Entry_ID", "ID", "Sample_condition_list_ID"):
                raise AssertionError("synthetic assigned-list columns drifted")
            return json.dumps(
                {entry_id: {key: [row[index] for row in values] for index, key in enumerate(ASSIGNED_RESPONSE_KEYS)}},
                sort_keys=True,
            ).encode("utf-8")
        return json.dumps(
...
def _failed_v3_intent_fixture(root: Path, raw: Optional[bytes] = None) -> dict[str, Any]:
    raw = _json_bytes(V3_FAILED_LAUNCH_INTENT) if raw is None else raw
    path = root / V3_FAILED_LAUNCH_INTENT_RELATIVE
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(raw)
    return {
        "path": V3_FAILED_LAUNCH_INTENT_RELATIVE.as_posix(),
...
def _run126_raw(controls: Optional[dict[str, Any]] = None) -> bytes:
    semantic = RUN126_LOG_BINDING["semantic_subset"]
    record = {
        "run": semantic["run"], "commit": semantic["commit"], "metric": semantic["metric"],
        "status": semantic["status"], "description": semantic["description"],
        "ASI": {
            "error": "ValueError: synthetic incorrect full commit binding",
            "failure_stage": "launch_commit_gate",
            "rollback_reason": "synthetic erroneous v3 launch commit",
            "observed_controls": controls if controls is not None else {
                "target_values_read": False, "network_requests": 0, "new_response_count_written": 0,
            },
        },
...
def _run126_binding(raw: bytes) -> dict[str, Any]:
    return {
        "path": LOG_RELATIVE.as_posix(),
        "raw_line_sha256": _sha256(raw + b"\n"),
        "semantic_subset": RUN126_LOG_BINDING["semantic_subset"],
    }
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        intent = _failed_v3_intent_fixture(root)
        assert _validate_failed_v3_launch_intent(root, intent) == intent
        _reject(
            lambda: _validate_failed_v3_launch_intent(root),
            "fixed failed-v3 intent SHA-256",
        )
        checks += 2
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        tampered = dict(V3_FAILED_LAUNCH_INTENT)
        tampered["git_commit"] = "0" * 40
        intent = _failed_v3_intent_fixture(root, _json_bytes(tampered))
        _reject(
            lambda: _validate_failed_v3_launch_intent(root, intent),
            "failed-v3 erroneous commit binding",
        )
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        binding126 = _run126_binding(raw)
        assert _validate_run126_log(root, binding126) == binding126
        _reject(lambda: _validate_run126_log(root), "fixed Run 126 raw SHA-256")
        log.write_bytes(raw)
        _reject(lambda: _validate_run126_log(root, binding126), "Run 126 missing final newline")
        checks += 3
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        raw = _run126_raw({"target_values_read": False, "network_requests": 1})
        log = root / LOG_RELATIVE
        log.parent.mkdir(parents=True)
        log.write_bytes(raw + b"\n")
        _reject(
            lambda: _validate_run126_log(root, _run126_binding(raw)),
            "Run 126 opened control",
        )
        checks += 1
    if False:  # superseded v4 fixture retained only as unreachable historical test text
        root = Path(".")
        _v1_fixture(root)
        v2_binding = _intent_fixture(root)
        v3_binding = _failed_v3_intent_fixture(root)
        run122 = _run122_raw()
        run126 = _run126_raw()
        log = root / LOG_RELATIVE
        log.parent.mkdir(parents=True, exist_ok=True)
        log.write_bytes(run122 + b"\n" + run126 + b"\n")
        consumed_raw = b'{"synthetic":"consumed-v4"}\n'
        consumed = root / V5_CONSUMED_INTENT_RELATIVE
        consumed.parent.mkdir(parents=True, exist_ok=True)
        consumed.write_bytes(consumed_raw)
        v4_directory = root / V5_FAILURE_RELATIVE
        (v4_directory / "raw_api_responses").mkdir(parents=True)
        v4_receipt_raw = _json_bytes(dict(V5_FAILURE_REQUIRED_FIELDS))
        v4_marker_raw = b"2026-09-06T05:00:00Z\n"
        (v4_directory / "failure_receipt.json").write_bytes(v4_receipt_raw)
        (v4_directory / "started_at_utc.txt").write_bytes(v4_marker_raw)
        v4_binding = {
            "failure_directory": V5_FAILURE_RELATIVE.as_posix(),
            "failure_inventory": {
                "regular_files": ["failure_receipt.json", "started_at_utc.txt"],
            ...
            "failure_receipt": {
                "path": "failure_receipt.json", "sha256": _sha256(v4_receipt_raw),
                "required_fields": V5_FAILURE_REQUIRED_FIELDS,
            },
            "started_at_utc": {"path": "started_at_utc.txt", "sha256": _sha256(v4_marker_raw)},
        }
        run131 = json.dumps(
            {
                "run": 131, "commit": "fe15f37", "metric": 0, "status": "crash",
                "description": RUN131_LOG_BINDING["semantic_subset"]["description"],
                "ASI": {"observed_controls": {"network_requests": 0, "target_values_read": False}},
            }, sort_keys=True, separators=(",", ":")
        ).encode("utf-8")
        with open(log, "ab") as handle:
            handle.write(run131 + b"\n")
        run131_binding = {
            "path": LOG_RELATIVE.as_posix(), "raw_line_sha256": _sha256(run131),
            "semantic_subset": RUN131_LOG_BINDING["semantic_subset"],
        }
        gates = {
            "actual_v1_failure_tree": V1_FAILURE_BINDING,
            "active_v2_launch_intent": v2_binding,
            "failed_v3_launch_intent": v3_binding,
            "run_122_log_record": {
                "path": LOG_RELATIVE.as_posix(),
                "raw_line_sha256": _sha256(run122),
                "semantic_subset": RUN122_LOG_BINDING["semantic_subset"],
            },
            "run_126_log_record": _run126_binding(run126),
            "v2_output_must_be_absent": V2_OUTPUT_RELATIVE.as_posix(),
            "v3_output_must_be_absent": V3_OUTPUT_RELATIVE.as_posix(),
            "consumed_v4_launch_intent": {
                "path": V5_CONSUMED_INTENT_RELATIVE.as_posix(), "sha256": _sha256(consumed_raw),
            },
            "actual_v4_failure_tree": v4_binding,
            "run_131_log_record": run131_binding,
            "active_v4_launch_intent_must_be_absent": V5_ACTIVE_INTENT_RELATIVE.as_posix(),
        }
        evidence = _validate_pre_output_gates(root, gates)
        assert evidence["legacy_recovery_evidence"]["failed_v3_launch_intent"] == v3_binding
        assert evidence["v4_failure_tree"] == v4_binding
        assert evidence["run_131_log_record"] == run131_binding
        assert V5_CONSUMED_INTENT_BINDING["sha256"] == "f625dbfdc61ac05b0825f89171331238636f9e5cd002cb3f6a6d0e9d0219895a"
        assert V5_FAILURE_BINDING["failure_receipt"]["sha256"] == "0c79b8564c804c93d49e07c0103b274829dfe5a21fef820ac7dcd11016b8f3d5"
        assert V5_FAILURE_BINDING["started_at_utc"]["sha256"] == "79246f3777035d2063217fec34be01b74479d9be7e337186dfbe7aff1ca71d5b"
        assert RUN131_LOG_BINDING["raw_line_sha256"] == "4e06e334dbb873b9c87e9df91c59df3d1c2cde1f623bb9b388d1f83694355451"
        checks += 1
        (v4_directory / "failure_receipt.json").write_bytes(b"tampered")
        _reject(lambda: _validate_v5_failure(root, v4_binding), "v4 failure receipt hash")
        checks += 1
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        failure = root / V5_FAILURE_RELATIVE
        (failure / "raw_api_responses").mkdir(parents=True)
        failure_raw = _json_bytes(dict(V5_FAILURE_REQUIRED_FIELDS))
        marker_raw = b"synthetic-v5-marker\n"
        (failure / "failure_receipt.json").write_bytes(failure_raw)
        (failure / "started_at_utc.txt").write_bytes(marker_raw)
        binding = {
            "failure_directory": V5_FAILURE_RELATIVE.as_posix(),
            "failure_inventory": {"regular_files": ["failure_receipt.json", "started_at_utc.txt"], "directories": [{"path": "raw_api_responses", "inventory": []}]},
            "failure_receipt": {"path": "failure_receipt.json", "sha256": _sha256(failure_raw), "required_fields": V5_FAILURE_REQUIRED_FIELDS},
            "started_at_utc": {"path": "started_at_utc.txt", "sha256": _sha256(marker_raw)},
        }
        assert _validate_v5_failure(root, binding) == binding
        (failure / "failure_receipt.json").write_bytes(b"tampered")
        _reject(lambda: _validate_v5_failure(root, binding), "v5 evidence tamper")
        checks += 2
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
    ...
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        (root / V3_OUTPUT_RELATIVE).mkdir(parents=True)
        _reject(lambda: _require_v3_output_absent(root), "v3 output presence")
        checks += 1
    ...
    experiment = _decode(prior["Experiment"], "synthetic Experiment response")
    experiment["42"]["Experiment"][0]["tags"].extend(
        ["Chromatographic_column_ID", "Chromatographic_column_label"]
    )
    experiment["42"]["Experiment"][0]["data"][0].extend(["1", "column"])
    assert _rows(json.dumps(experiment).encode("utf-8"), "42", "Experiment")
    checks += 1
    experiment["42"]["Experiment"][0]["tags"].append("Unknown_metadata")
    experiment["42"]["Experiment"][0]["data"][0].append("x")
    _reject(
        lambda: _rows(json.dumps(experiment).encode("utf-8"), "42", "Experiment"),
        "unknown Experiment tag",
    )
    checks += 1
    parsed = _decode(assigned, "synthetic response")
    parsed["42"]["_Atom_chem_shift.Val"] = ["target"]
    _reject(lambda: _rows(json.dumps(parsed).encode("utf-8"), "42", NEW_LOOP), "target-bearing key")
    parsed = _decode(assigned, "synthetic response")
    parsed["42"][ASSIGNED_RESPONSE_KEYS[1]].append("8")
    _reject(lambda: _rows(json.dumps(parsed).encode("utf-8"), "42", NEW_LOOP), "column length mismatch")
    checks += 2
    ...
    with tempfile.TemporaryDirectory() as temporary:
        _reject(
            lambda: _validate_receipt_static(
                receipt, Path(temporary), "0" * 40, "0" * 40, LEGACY_RECOVERY_EVIDENCE
            ),
            "receipt tamper",
        )
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        os.unlink(raw_directory / expected)
        os.symlink("../receipt.json", raw_directory / expected)
        _reject(
            lambda: _validate_artifact_tree(root, "2026-01-01T00:00:00Z", {expected}),
            "output response symlink",
        )
        os.unlink(raw_directory / expected)
        (raw_directory / expected).write_bytes(assigned)
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        checks += 2
    with tempfile.TemporaryDirectory() as temporary:
        receipt = Path(temporary) / "independent-receipt.json"
        descriptor = os.open(str(receipt), os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        os.write(descriptor, b"first")
        os.close(descriptor)
        try:
            os.open(str(receipt), os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        except FileExistsError:
            checks += 1
        else:
            raise AssertionError("independent receipt clobber was accepted")
    return checks
...
def main() -> int:
    ...
    if args.command == "self-test":
        checks = self_test()
        print("METRIC openmm86_deposited_ph_recovery_v6_checker_self_tests=%d" % checks)
        print("STATUS PASS")
        return 0
    ...
    try:
        ...
        print("REFUSAL fixed local recovery-v6 evidence is absent or invalid: %s" % error)
        print("STATUS HOLD_DEPOSITED_PH_METADATA_INCOMPLETE_OR_AMBIGUOUS")
        return 3
    print("METRIC openmm86_deposited_ph_recovery_v6_checker_checks=%d" % checks)
