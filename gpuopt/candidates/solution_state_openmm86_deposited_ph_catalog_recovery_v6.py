...
V3_FAILED_LAUNCH_INTENT = {
    ...
    "checker": {
        "path": (
            "gpuopt/candidates/"
            "check_solution_state_openmm86_deposited_ph_catalog_recovery_v3.py"
        ),
        "sha256": "ecb5cc73bfc09be99ab3891fbd0d67d091925c0b8be13abe4f720ec11a64bf7d",
    },
    "v1_failure_receipt_sha256": V1_FAILURE_RECEIPT_SHA256,
    "v2_launch_intent_sha256": V2_LAUNCH_INTENT_BINDING["sha256"],
...
RUN126_LOG_BINDING = {
    ...
    "semantic_subset": {
        ...
        "status": "crash",
        "description": (
            "Run 126: recovery-v3 launch failed closed on incorrect full "
            "commit binding"
        ),
        "asi_required_fields": [
            "error", "failure_stage", "observed_controls", "rollback_reason"
        ],
        "observed_controls": "all leaves are false or integer zero",
    },
}
...
# Recovery-v6 follows the consumed v5 launch-v2.  The first v5 request failed
# before any response was persisted, so the sealed tree, primary terminal
# receipt, and Run 136 record are admission evidence rather than retry input.
# Every v5 and Run 136 admission artifact is bound by its full SHA-256.
V5_CONSUMED_INTENT_RELATIVE = Path(
    ".auto/staging/"
...
V5_EXECUTION_RECEIPT_RELATIVE = Path(
    ".auto/staging/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_"
    "execution_receipt_v2.json"
)
V5_FALLBACK_RECEIPT_RELATIVE = Path(
    ".auto/staging/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_"
...
V5_CONSUMED_INTENT_BINDING = {
    "path": V5_CONSUMED_INTENT_RELATIVE.as_posix(),
    "sha256": V5_CONSUMED_INTENT_SHA256,
}
V5_FAILURE_BINDING = {
    ...
    "failure_receipt": {
        "path": "failure_receipt.json",
        "sha256": V5_FAILURE_RECEIPT_SHA256,
        "required_fields": V5_FAILURE_REQUIRED_FIELDS,
    },
    "started_at_utc": {
        "path": "started_at_utc.txt",
        "sha256": V5_STARTED_AT_SHA256,
    },
}
V5_EXECUTION_RECEIPT_BINDING = {
    "path": V5_EXECUTION_RECEIPT_RELATIVE.as_posix(),
    "sha256": V5_EXECUTION_RECEIPT_SHA256,
}
RUN136_LOG_BINDING = {
    "path": LOG_RELATIVE.as_posix(),
    "raw_line_sha256": RUN136_RAW_LINE_SHA256,
    "semantic_subset": {"run": 136, "status": "crash"},
}
PRE_OUTPUT_GATES = {
    **LEGACY_PRE_OUTPUT_GATES,
...
# Exact archive observation: all 46 non-target Experiment tags were present.
...
def _read_failed_v3_launch_intent(
    root: Path, binding: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """Bind the sealed failed v3 launch rather than retrying it in place."""
    ...
    if not (
        ...
        and isinstance(binding.get("sha256"), str)
        and re.fullmatch(r"[0-9a-f]{64}", binding["sha256"])
        and _exact_json(binding.get("exact_fields"), V3_FAILED_LAUNCH_INTENT)
    ):
        raise ValueError("failed v3 launch-intent binding drifted")
    raw = _read_regular(
        root,
        V3_FAILED_LAUNCH_INTENT_RELATIVE,
        "failed v3 launch intent",
        MAX_SOURCE_BYTES,
    )
    if _sha256(raw) != binding["sha256"]:
        raise ValueError("failed v3 launch intent raw SHA-256 drifted")
    if not _exact_json(
        _loads(raw, "failed v3 launch intent"), binding["exact_fields"]
    ):
        raise ValueError("failed v3 launch intent schema, path, or values drifted")
    return binding
...
def _run126_asi(record: Dict[str, Any]) -> Dict[str, Any]:
    required = set(RUN126_LOG_BINDING["semantic_subset"]["asi_required_fields"])
    matches: List[Dict[str, Any]] = []
    ...
    for name in ("error", "failure_stage", "rollback_reason"):
        if not isinstance(asi[name], str) or not asi[name]:
            raise ValueError("Run 126 ASI %s drifted" % name)
    if not isinstance(asi["observed_controls"], dict) or not _closed_control_tree(
        asi["observed_controls"]
    ):
...
def _validate_run126_record(raw: bytes, expected_sha256: str) -> None:
    if _sha256(raw) != expected_sha256:
        raise ValueError("Run 126 raw log-line SHA-256 drifted")
    record = _loads(raw, "Run 126 log line")
...
def _read_run126_log_evidence(
    root: Path, binding: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """Bind exactly one Run 126 JSONL record, including its terminating LF."""
    ...
    if not (
        ...
        and re.fullmatch(r"[0-9a-f]{64}", binding["raw_line_sha256"])
        and _exact_json(
            binding.get("semantic_subset"), RUN126_LOG_BINDING["semantic_subset"]
        )
    ):
    ...
    _validate_run126_record(matches[0], binding["raw_line_sha256"])
    return binding


def _require_output_absent(root: Path, relative: Path, label: str) -> None:
    """Reject a prior output (including an indirect path) before v4 exists."""
    _safe_relative(relative)
    current = root
    for index, piece in enumerate(relative.parts):
        ...
        try:
            ...
            raise ValueError("cannot inspect %s output absence gate" % label) from error
        if stat.S_ISLNK(details.st_mode):
            raise ValueError("%s output absence gate is indirect" % label)
        if index == len(relative.parts) - 1:
            raise ValueError("%s output already exists; no in-place retry is permitted" % label)
        if not stat.S_ISDIR(details.st_mode):
            raise ValueError("%s output parent is not a directory" % label)
...
def _validate_legacy_pre_output_gates(
    ...
    """Validate each v1/v2/v3 recovery gate before any later read or output."""
    gates = LEGACY_PRE_OUTPUT_GATES if gates is None else gates
    if not (
        isinstance(gates, dict)
        and set(gates) == set(LEGACY_PRE_OUTPUT_GATES)
        and _exact_json(gates.get("actual_v1_failure_tree"), V1_FAILURE_BINDING)
        ...
        raise ValueError("pre-output gate schema or fixed paths drifted")
    v1_failure = _read_v1_failure_evidence(root)
    v2_intent = _read_v2_launch_intent(root, gates["active_v2_launch_intent"])
    failed_v3_intent = _read_failed_v3_launch_intent(
        root, gates["failed_v3_launch_intent"]
    )
    run122 = _read_run122_log_evidence(root, gates["run_122_log_record"])
    run126 = _read_run126_log_evidence(root, gates["run_126_log_record"])
    _require_v2_output_absent(root)
    _require_v3_output_absent(root)
    evidence = {
        "v1_failure_tree": v1_failure,
        "v2_launch_intent": v2_intent,
        "failed_v3_launch_intent": failed_v3_intent,
        "run_122_log_record": run122,
        "run_126_log_record": run126,
        "v2_output_absent_before_v4_creation": V2_OUTPUT_RELATIVE.as_posix(),
        "v3_output_absent_before_v4_creation": V3_OUTPUT_RELATIVE.as_posix(),
    }
    expected = LEGACY_RECOVERY_EVIDENCE if gates is LEGACY_PRE_OUTPUT_GATES else evidence
    if not _exact_json(evidence, expected):
        raise ValueError("pre-output recovery evidence drifted")
    return evidence


def _required_json(value: Any, required: Any) -> bool:
    """Strictly compare required JSON leaves while permitting provenance extras."""
    if type(value) is not type(required):
        return False
    if isinstance(required, dict):
        return all(key in value and _required_json(value[key], wanted) for key, wanted in required.items())
    if isinstance(required, list):
        return len(value) == len(required) and all(
            _required_json(item, wanted) for item, wanted in zip(value, required)
        )
    return value == required


def _v5_failure_directory(root: Path) -> Path:
    _safe_relative(V5_FAILURE_RELATIVE)
    directory = root
    for piece in V5_FAILURE_RELATIVE.parts:
        directory = directory / piece
        try:
            details = os.lstat(directory)
        except OSError as error:
            raise ValueError("missing sealed v5 failure directory") from error
        if stat.S_ISLNK(details.st_mode) or not stat.S_ISDIR(details.st_mode):
            raise ValueError("sealed v5 failure directory is indirect")
    return directory


def _matches_hash_attestation(raw: bytes, spec: Dict[str, Any], label: str) -> None:
    """Require one exact full-digest attestation."""
    if set(spec) - {"path", "sha256", "required_fields"}:
        raise ValueError("%s attestation schema drifted" % label)
    full = spec.get("sha256")
    digest = _sha256(raw)
    if (
        not isinstance(full, str)
        or re.fullmatch(r"[0-9a-f]{64}", full) is None
        or digest != full
    ):
        raise ValueError("%s raw SHA-256 drifted" % label)


def _read_v5_consumed_intent(root: Path, binding: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if not (
        ...
        and isinstance(binding.get("sha256"), str)
        and re.fullmatch(r"[0-9a-f]{64}", binding["sha256"])
        and (not fixed or _exact_json(binding, V5_CONSUMED_INTENT_BINDING))
    ):
        raise ValueError("consumed v5 launch-v2 binding drifted")
    raw = _read_regular(root, V5_CONSUMED_INTENT_RELATIVE, "consumed v5 launch-v2", MAX_SOURCE_BYTES)
    if _sha256(raw) != binding["sha256"]:
        raise ValueError("consumed v5 launch-v2 raw SHA-256 drifted")
    return binding


def _read_v5_failure_evidence(root: Path, binding: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    fixed = binding is None
    ...
    if not (
        ...
        and _required_json(binding.get("failure_receipt"), {"path": "failure_receipt.json", "required_fields": V5_FAILURE_REQUIRED_FIELDS})
        and _required_json(binding.get("started_at_utc"), {"path": "started_at_utc.txt"})
        and (not fixed or _exact_json(binding, V5_FAILURE_BINDING))
    ):
        raise ValueError("sealed v5 failure-tree binding drifted")
    directory = _v5_failure_directory(root)
    inventory = binding["failure_inventory"]
    if not isinstance(inventory, dict) or inventory.get("regular_files") != ["failure_receipt.json", "started_at_utc.txt"] or inventory.get("directories") != [{"path": "raw_api_responses", "inventory": []}]:
        raise ValueError("sealed v5 failure inventory binding drifted")
    try:
        with os.scandir(directory) as entries:
            files, directories = set(), set()
            for entry in entries:
                if entry.is_symlink():
                    raise ValueError("sealed v5 failure inventory has a symlink")
                if entry.is_file(follow_symlinks=False):
                    files.add(entry.name)
                elif entry.is_dir(follow_symlinks=False):
                    directories.add(entry.name)
                else:
                    raise ValueError("sealed v5 failure inventory has an indirect entry")
    except OSError as error:
        raise ValueError("cannot inspect sealed v5 failure directory") from error
    if files != {"failure_receipt.json", "started_at_utc.txt"} or directories != {"raw_api_responses"}:
        raise ValueError("sealed v5 failure inventory drifted")
    try:
        with os.scandir(directory / "raw_api_responses") as entries:
            if any(True for _ in entries):
                raise ValueError("sealed v5 raw-response directory is not exactly empty")
    except OSError as error:
        raise ValueError("cannot inspect sealed v5 raw-response directory") from error
    receipt_raw = _read_regular(root, V5_FAILURE_RELATIVE / "failure_receipt.json", "sealed v5 failure receipt", MAX_RESPONSE_BYTES)
    marker_raw = _read_regular(root, V5_FAILURE_RELATIVE / "started_at_utc.txt", "sealed v5 start marker", 1024)
    _matches_hash_attestation(receipt_raw, binding["failure_receipt"], "sealed v5 failure receipt")
    _matches_hash_attestation(marker_raw, binding["started_at_utc"], "sealed v5 start marker")
    if not _required_json(_loads(receipt_raw, "sealed v5 failure receipt"), binding["failure_receipt"]["required_fields"]):
        raise ValueError("sealed v5 failure receipt fields drifted")
    return binding


def _read_v5_execution_receipt(root: Path, binding: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    fixed = binding is None
    ...
    if not (
        ...
        and (not fixed or _exact_json(binding, V5_EXECUTION_RECEIPT_BINDING))
    ):
        raise ValueError("v5 execution-receipt binding drifted")
    raw = _read_regular(root, V5_EXECUTION_RECEIPT_RELATIVE, "v5 execution receipt", MAX_RESPONSE_BYTES)
    _matches_hash_attestation(raw, binding, "v5 execution receipt")
    receipt = _loads(raw, "v5 execution receipt")
...
def _read_run136_log_evidence(root: Path, binding: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    fixed = binding is None
    ...
    if not (
        ...
        and isinstance(binding.get("raw_line_sha256"), str)
        and re.fullmatch(r"[0-9a-f]{64}", binding["raw_line_sha256"])
        and _exact_json(binding.get("semantic_subset"), {"run": 136, "status": "crash"})
        and (not fixed or _exact_json(binding, RUN136_LOG_BINDING))
    ):
    ...
    if len(lines) > MAX_LOG_LINES:
        raise ValueError("append-only operational log exceeds the line limit")
    matches = [line for line in lines if _sha256(line) == binding["raw_line_sha256"]]
    if len(matches) != 1:
        raise ValueError("Run 136 bound raw log line is absent or duplicated")
    record = _loads(matches[0], "Run 136 log line")
...
def _validate_pre_output_gates(root: Path, gates: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Validate inherited evidence and the sealed v5 first-request failure."""
    fixed = gates is None
    ...
    if not (
        ...
        and (not fixed or _exact_json(gates, PRE_OUTPUT_GATES))
    ):
    ...
    consumed = _read_v5_consumed_intent(root, gates["consumed_v5_launch_intent"])
    failure = _read_v5_failure_evidence(root, gates["actual_v5_failure_tree"])
    execution = _read_v5_execution_receipt(root, gates["v5_execution_receipt"])
    run136 = _read_run136_log_evidence(root, gates["run_136_log_record"])
    ...
    evidence = {"legacy_recovery_evidence": legacy, "consumed_v5_launch_intent": consumed, "v5_failure_tree": failure, "v5_execution_receipt": execution, "run_136_log_record": run136}
    if fixed and not _exact_json(evidence, RECOVERY_EVIDENCE):
        raise ValueError("recovery-v6 pre-output recovery evidence drifted")
...
def _load_plan(root: Path) -> Tuple[bytes, Dict[str, Any], str]:
    ...
    if not isinstance(plan, dict) or not (
        ...
        and _exact_json(plan.get("recovery_of"), RECOVERY_EVIDENCE)
        ...
        rows = [
            {
                "Entry_ID": values[0],
                "ID": values[1],
                "Sample_condition_list_ID": values[2],
            }
...
def _experiment_tags(payload: bytes, entry_id: str) -> frozenset[str]:
    """Return one sealed Experiment response's tags after normal admission."""
    _rows(payload, entry_id, "Experiment")
    decoded = _loads(payload, "BMRB Experiment response")
    entry = decoded[entry_id]
    if entry == {}:
        return frozenset()
    return frozenset(
...
def _fetch_assigned_list(entry_id: str) -> Tuple[bytes, str, str]:
    if ID_RE.fullmatch(entry_id) is None:
        raise ValueError("assigned-list entry ID is invalid")
    # BMRB v2 metadata route: ordered repeated tag parameters, never tag_list
    # and never the invalid category sub-route used by recovery-v5.
    requested_url = "%s/entry/%s?%s" % (
        API_BASE,
        entry_id,
...
def fetch_catalog() -> Dict[str, Any]:
    root = _bound_root()
    # All v1/v2/v3 admission evidence is validated before output creation,
        ...
        with zipfile.ZipFile(io.BytesIO(archive_raw)) as archive:
            ...
            for entity in roster:
                ...
                observed_experiment_tags.update(
                    _experiment_tags(prevalidated_prior[entity_uid][0]["Experiment"], bmrb_id[3:])
                )
            ...
            return json.dumps(
                {
                    entry_id: {
                        ...
                        for index, key in enumerate(ASSIGNED_RESPONSE_KEYS)
                    }
                },
                sort_keys=True,
            ).encode("utf-8")
...
def _write_failed_v3_intent_fixture(
    root: Path, raw: Optional[bytes] = None
) -> Dict[str, Any]:
...
def _write_run126_log_fixture(root: Path, raw: Optional[bytes] = None) -> Dict[str, Any]:
    raw = _run126_fixture_raw() if raw is None else raw
    line = raw + b"\n"
    path = root / LOG_RELATIVE
    path.parent.mkdir(parents=True, exist_ok=True)
    _write_new(path, line)
    return {
        "path": LOG_RELATIVE.as_posix(),
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        binding = _write_failed_v3_intent_fixture(root)
        assert _read_failed_v3_launch_intent(root, binding) == binding
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        path = root / V3_FAILED_LAUNCH_INTENT_RELATIVE
        os.unlink(path)
        _write_new(path, b"failed-v3 intent hash tamper")
        _expect_value_error(
            lambda: _read_failed_v3_launch_intent(root, binding),
            "failed v3 intent hash tamper",
        )
        checks += 1

    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        tampered = json.loads(json.dumps(V3_FAILED_LAUNCH_INTENT))
        tampered["git_commit"] = "0" * 40
        binding = _write_failed_v3_intent_fixture(root, _json_bytes(tampered))
        _expect_value_error(
            lambda: _read_failed_v3_launch_intent(root, binding),
            "failed v3 intent schema tamper",
        )
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        _write_v1_failure_fixture(root)
        v2_binding = _write_v2_intent_fixture(root)
        failed_v3_binding = _write_failed_v3_intent_fixture(root)
        run122_raw, run126_raw = _run122_fixture_raw(), _run126_fixture_raw()
        run136_raw = b'{"run":136,"status":"crash"}'
        log = root / LOG_RELATIVE
        log.parent.mkdir(parents=True, exist_ok=True)
        _write_new(log, run122_raw + b"\n" + run126_raw + b"\n" + run136_raw + b"\n")
        consumed_raw = b'{"synthetic":"consumed-v5"}\n'
        consumed_path = root / V5_CONSUMED_INTENT_RELATIVE
        consumed_path.parent.mkdir(parents=True, exist_ok=True)
        _write_new(consumed_path, consumed_raw)
        v5_directory = root / V5_FAILURE_RELATIVE
        v5_directory.mkdir(parents=True)
        (v5_directory / "raw_api_responses").mkdir()
        v5_receipt_raw = _json_bytes(dict(V5_FAILURE_REQUIRED_FIELDS))
        v5_marker_raw = b"2026-09-06T05:00:00Z\n"
        _write_new(v5_directory / "failure_receipt.json", v5_receipt_raw)
        _write_new(v5_directory / "started_at_utc.txt", v5_marker_raw)
        execution_raw = _json_bytes({
            "candidate_id": "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5",
            "contract": "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v5_execution_v2",
            "outcome": "producer_failed", "closed_controls": CLOSED_FLAGS,
        })
        execution_path = root / V5_EXECUTION_RECEIPT_RELATIVE
        _write_new(execution_path, execution_raw)
        v5_binding = {
            "failure_directory": V5_FAILURE_RELATIVE.as_posix(),
            "failure_inventory": {"regular_files": ["failure_receipt.json", "started_at_utc.txt"], "directories": [{"path": "raw_api_responses", "inventory": []}]},
            "failure_receipt": {"path": "failure_receipt.json", "sha256": _sha256(v5_receipt_raw), "required_fields": V5_FAILURE_REQUIRED_FIELDS},
            "started_at_utc": {"path": "started_at_utc.txt", "sha256": _sha256(v5_marker_raw)},
        }
        gates = {
            "actual_v1_failure_tree": V1_FAILURE_BINDING,
            "active_v2_launch_intent": v2_binding,
            "failed_v3_launch_intent": failed_v3_binding,
            "run_122_log_record": {"path": LOG_RELATIVE.as_posix(), "raw_line_sha256": _sha256(run122_raw), "semantic_subset": RUN122_LOG_BINDING["semantic_subset"]},
            "run_126_log_record": {"path": LOG_RELATIVE.as_posix(), "raw_line_sha256": _sha256(run126_raw + b"\n"), "semantic_subset": RUN126_LOG_BINDING["semantic_subset"]},
            "v2_output_must_be_absent": V2_OUTPUT_RELATIVE.as_posix(),
            "v3_output_must_be_absent": V3_OUTPUT_RELATIVE.as_posix(),
            "consumed_v5_launch_intent": {"path": V5_CONSUMED_INTENT_RELATIVE.as_posix(), "sha256": _sha256(consumed_raw)},
            "actual_v5_failure_tree": v5_binding,
            "v5_execution_receipt": {"path": V5_EXECUTION_RECEIPT_RELATIVE.as_posix(), "sha256": _sha256(execution_raw)},
            "run_136_log_record": {"path": LOG_RELATIVE.as_posix(), "raw_line_sha256": _sha256(run136_raw + b"\n"), "semantic_subset": {"run": 136, "status": "crash"}},
            "active_v5_launch_intent_must_be_absent": V5_ACTIVE_INTENT_RELATIVE.as_posix(),
            "v5_success_receipt_must_be_absent": V5_SUCCESS_RECEIPT_RELATIVE.as_posix(),
            "v5_fallback_execution_receipt_must_be_absent": V5_FALLBACK_RECEIPT_RELATIVE.as_posix(),
        }
        evidence = _validate_pre_output_gates(root, gates)
        assert evidence["v5_failure_tree"] == v5_binding
        assert evidence["v5_execution_receipt"] == gates["v5_execution_receipt"]
        checks += 1
        os.unlink(v5_directory / "failure_receipt.json")
        _write_new(v5_directory / "failure_receipt.json", b"tampered")
        _expect_value_error(lambda: _read_v5_failure_evidence(root, v5_binding), "v5 failure receipt hash")
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        binding = _write_run126_log_fixture(root)
        assert _read_run126_log_evidence(root, binding) == binding
        checks += 1

    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        raw = _run126_fixture_raw()
        binding = _write_run126_log_fixture(root, raw)
        path = root / LOG_RELATIVE
        os.unlink(path)
        _write_new(path, raw + b" \n")
        _expect_value_error(lambda: _read_run126_log_evidence(root, binding), "Run 126 hash tamper")
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        os.unlink(path)
        _write_new(path, raw + b"\n" + raw + b"\n")
        _expect_value_error(lambda: _read_run126_log_evidence(root, binding), "Run 126 duplicate")
        checks += 1

    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        raw = _run126_fixture_raw()
        record = _loads(raw, "synthetic Run 126")
        del record["status"]
        binding = _write_run126_log_fixture(root, _json_bytes(record).rstrip(b"\n"))
        _expect_value_error(lambda: _read_run126_log_evidence(root, binding), "Run 126 schema")
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        raw = _run126_fixture_raw()
        record = _loads(raw, "synthetic Run 126")
        record["ASI"]["observed_controls"]["network_requests"] = 1
        binding = _write_run126_log_fixture(root, _json_bytes(record).rstrip(b"\n"))
        _expect_value_error(
            lambda: _read_run126_log_evidence(root, binding), "Run 126 closed controls"
        )
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        path.parent.mkdir(parents=True)
        _write_new(path, b"x" * (MAX_LOG_BYTES + 1))
        _expect_value_error(lambda: _read_run126_log_evidence(root), "Run 126 bounded log read")
        checks += 1
    ...
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        output = root / V3_OUTPUT_RELATIVE
        output.mkdir(parents=True)
        _expect_value_error(lambda: _require_v3_output_absent(root), "v3 output presence")
        assert not (root / OUTPUT_RELATIVE).exists()
        checks += 1
    ...
    prior, assigned = _fixture()
    experiment = _loads(prior["Experiment"], "synthetic Experiment response")
    instance = experiment["42"]["Experiment"][0]
    instance["tags"].extend(
        ["Chromatographic_column_ID", "Chromatographic_column_label"]
    )
    instance["data"][0].extend(["1", "column"])
    assert _rows(json.dumps(experiment).encode("utf-8"), "42", "Experiment")
    checks += 1
    instance["tags"].append("Unknown_metadata")
    instance["data"][0].append("x")
    _expect_value_error(
    ...
    _expect_value_error(
        ...
        "extra target-bearing assigned-list key",
    )
    checks += 1
    parsed = _loads(assigned, "synthetic assigned response")
    ...
    _expect_value_error(
        lambda: _rows(json.dumps(parsed).encode("utf-8"), "42", NEW_LOOP),
        "assigned-list column length mismatch",
    )
    checks += 1
    parsed = _loads(assigned, "synthetic assigned response")
    parsed["42"][ASSIGNED_RESPONSE_KEYS[0]][0] = 42
    _expect_value_error(
        lambda: _rows(json.dumps(parsed).encode("utf-8"), "42", NEW_LOOP),
        "assigned-list non-string scalar",
    )
    checks += 1

    expected_url = API_BASE + "/entry/42?" + urllib.parse.urlencode(
        [("tag", key) for key in ASSIGNED_RESPONSE_KEYS]
    )
    _require_exact_official_url(expected_url, expected_url)
    _expect_value_error(
        lambda: _require_exact_official_url(
            "http://api.bmrb.io/v2/entry/42?" + urllib.parse.urlencode([("tag", key) for key in ASSIGNED_RESPONSE_KEYS]),
            expected_url,
        ),
    ...
    _expect_value_error(
        lambda: _require_exact_official_url(
            "https://api.bmrb.io:444/v2/entry/42?" + urllib.parse.urlencode([("tag", key) for key in ASSIGNED_RESPONSE_KEYS]),
            expected_url,
        ),
        "nonstandard official URL port",
    )
    _expect_value_error(
        lambda: _require_exact_official_url(
            "https://api.bmrb.io/v2/entry/42?tag=_Assigned_chem_shift_list.ID&tag=_Assigned_chem_shift_list.Entry_ID&tag=_Assigned_chem_shift_list.Sample_condition_list_ID",
            expected_url,
        ),
        "redirected official URL query",
    )
    checks += 1
...
def main() -> int:
    ...
    if args.command == "self-test":
        checks = self_test()
        print("METRIC openmm86_deposited_ph_recovery_v6_checks=%d" % checks)
        print("STATUS PASS")
        return 0
    ...
    try:
        ...
        print("STATUS CRASH_OPENMM86_DEPOSITED_PH_RECOVERY_V6")
        print("ERROR %s: %s" % (type(error).__name__, error))
        return 1
    print("METRIC openmm86_deposited_ph_recovery_v6_feasible_entities=%d/%d" % (
