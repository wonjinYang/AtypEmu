...
"""Independently replay the fixed local deposited-pH recovery-v5 output.
...
CHECKER_RELATIVE = Path(
    "gpuopt/candidates/check_solution_state_openmm86_deposited_ph_catalog_recovery_v5.py"
)
PLAN_RELATIVE = Path(
    "gpuopt/preunblind/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v5_plan.json"
)
...
ARTIFACT_RELATIVE = Path(
    ".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_api_v5_recovery"
)
...
PLAN_CONTRACT = "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v5_plan"
API_BASE = "https://api.bmrb.io/v2"
APPLICATION_HEADER = "AtypEmu openmm86-deposited-ph-catalog-recovery-v5"
ENTITY_COUNT = 135
...
# Copied from the raw-byte-bound recovery-v5 plan.  These are fixed local
...
# v5 adds only the two archive-present, non-target Experiment column tags.
# These bindings seal the consumed v4 launch and failure before any v5 replay.
V4_CONSUMED_INTENT_RELATIVE = Path(
    ".auto/staging/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v4_"
    "launch_intent.consumed.json"
)
V4_ACTIVE_INTENT_RELATIVE = Path(
    ".auto/staging/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_recovery_v4_"
    "launch_intent.json"
)
...
V4_FAILURE_REQUIRED_FIELDS = {
    ...
    "contract": "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v4",
    "error": "ValueError: BMRB tags are incomplete, nonallowlisted, or target-bearing: Experiment",
    "new_response_count_written": 0,
    "new_response_manifest": [],
    "recovery_evidence": LEGACY_RECOVERY_EVIDENCE,
    **CLOSED_FLAGS,
}
V4_CONSUMED_INTENT_BINDING = {
    "path": V4_CONSUMED_INTENT_RELATIVE.as_posix(),
    "sha256": "f625dbfdc61ac05b0825f89171331238636f9e5cd002cb3f6a6d0e9d0219895a",
}
V4_FAILURE_BINDING = {
    "failure_directory": V4_FAILURE_RELATIVE.as_posix(),
    "failure_inventory": {
        "regular_files": ["failure_receipt.json", "started_at_utc.txt"],
        "directories": [{"path": "raw_api_responses", "inventory": []}],
    },
    "failure_receipt": {
        "path": "failure_receipt.json",
        "sha256": "0c79b8564c804c93d49e07c0103b274829dfe5a21fef820ac7dcd11016b8f3d5",
        "required_fields": V4_FAILURE_REQUIRED_FIELDS,
    },
    "started_at_utc": {
        "path": "started_at_utc.txt",
        "sha256": "79246f3777035d2063217fec34be01b74479d9be7e337186dfbe7aff1ca71d5b",
    },
}
RUN131_LOG_BINDING = {
    "path": LOG_RELATIVE.as_posix(),
    "raw_line_sha256": "4e06e334dbb873b9c87e9df91c59df3d1c2cde1f623bb9b388d1f83694355451",
    "semantic_subset": {
        "run": 131,
        "commit": "fe15f37",
        "metric": 0,
        "status": "crash",
        "description": (
            "Run 131: deposited-pH recovery-v4 launch failed after intent "
            "consumption before metadata output"
        ),
        "observed_controls": "all leaves are false or integer zero",
    },
}
PRE_OUTPUT_GATES = {
    **LEGACY_PRE_OUTPUT_GATES,
    "consumed_v4_launch_intent": V4_CONSUMED_INTENT_BINDING,
    "actual_v4_failure_tree": V4_FAILURE_BINDING,
    "run_131_log_record": RUN131_LOG_BINDING,
    "active_v4_launch_intent_must_be_absent": V4_ACTIVE_INTENT_RELATIVE.as_posix(),
}
RECOVERY_EVIDENCE = {
    "legacy_recovery_evidence": LEGACY_RECOVERY_EVIDENCE,
    "consumed_v4_launch_intent": V4_CONSUMED_INTENT_BINDING,
    "v4_failure_tree": V4_FAILURE_BINDING,
    "run_131_log_record": RUN131_LOG_BINDING,
}
...
# Exact archive-wide, non-target Experiment metadata observation for v5.
...
def _load_plan_and_producer(root: Path) -> tuple[dict[str, Any], str, str]:
    plan_head = _committed_exact(root, PLAN_RELATIVE, PLAN_RAW_SHA256, "recovery-v5 plan")
    producer_head = _committed_exact(
        root, PRODUCER_RELATIVE, PRODUCER_RAW_SHA256, "recovery-v5 producer"
    )
    plan = _decode(
        _read_regular(root, PLAN_RELATIVE, "recovery-v5 plan", MAX_SOURCE_BYTES),
        "recovery-v5 plan",
    )
    if not isinstance(plan, dict):
        raise ValueError("recovery-v5 plan root is not an object")
    ...
    if (
        ...
        raise ValueError("recovery-v5 plan semantic binding drifted")
...
def _validate_v1_failure(root: Path, binding: Any = V1_FAILURE_BINDING) -> dict[str, Any]:
    ...
    if not _exact(binding, V1_FAILURE_BINDING):
        raise ValueError("recovery-v5 v1 failure binding drifted")
...
def _validate_legacy_pre_output_gates(
    ...
    if not (
        ...
        raise ValueError("recovery-v5 pre-output gate schema or fixed paths drifted")
    evidence = {
    ...
    if fixed and not _exact(evidence, LEGACY_RECOVERY_EVIDENCE):
        raise ValueError("recovery-v5 pre-output evidence drifted")
...
def _v4_failure_directory(root: Path) -> Path:
    return _directory_path(root, V4_FAILURE_RELATIVE, "sealed v4 failure directory")


def _validate_v4_consumed_intent(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    fixed = binding is None
    binding = V4_CONSUMED_INTENT_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict) and set(binding) == {"path", "sha256"}
        and binding.get("path") == V4_CONSUMED_INTENT_RELATIVE.as_posix()
        and isinstance(binding.get("sha256"), str) and SHA256_RE.fullmatch(binding["sha256"])
        and (not fixed or _exact(binding, V4_CONSUMED_INTENT_BINDING))
    ):
        raise ValueError("consumed v4 launch-intent binding drifted")
    raw = _read_regular(root, V4_CONSUMED_INTENT_RELATIVE, "consumed v4 launch intent", MAX_SOURCE_BYTES)
    if _sha256(raw) != binding["sha256"]:
        raise ValueError("consumed v4 launch intent raw SHA-256 drifted")
    return binding


def _validate_v4_failure(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    fixed = binding is None
    binding = V4_FAILURE_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict)
        and set(binding) == {"failure_directory", "failure_inventory", "failure_receipt", "started_at_utc"}
        and binding.get("failure_directory") == V4_FAILURE_RELATIVE.as_posix()
        and _required(binding.get("failure_receipt"), {
            "path": "failure_receipt.json", "required_fields": V4_FAILURE_REQUIRED_FIELDS,
        })
        and _required(binding.get("started_at_utc"), {"path": "started_at_utc.txt"})
        and (not fixed or _exact(binding, V4_FAILURE_BINDING))
    ):
        raise ValueError("sealed v4 failure-tree binding drifted")
    receipt_spec = binding["failure_receipt"]
    marker_spec = binding["started_at_utc"]
    if not (
        isinstance(receipt_spec.get("sha256"), str) and SHA256_RE.fullmatch(receipt_spec["sha256"])
        and isinstance(marker_spec.get("sha256"), str) and SHA256_RE.fullmatch(marker_spec["sha256"])
    ):
        raise ValueError("sealed v4 failure-tree hash binding drifted")
    directory = _v4_failure_directory(root)
    inventory = binding["failure_inventory"]
    ...
    receipt_raw = _read_regular(root, V4_FAILURE_RELATIVE / "failure_receipt.json", "sealed v4 failure receipt", MAX_JSON_BYTES)
    marker_raw = _read_regular(root, V4_FAILURE_RELATIVE / "started_at_utc.txt", "sealed v4 start marker", 1024)
    if _sha256(receipt_raw) != receipt_spec["sha256"] or _sha256(marker_raw) != marker_spec["sha256"]:
        raise ValueError("sealed v4 failure raw SHA-256 drifted")
    if not _required(_decode(receipt_raw, "sealed v4 failure receipt"), receipt_spec["required_fields"]):
        raise ValueError("sealed v4 failure receipt fields drifted")
    return binding


def _validate_run131_log(
    root: Path, binding: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    fixed = binding is None
    binding = RUN131_LOG_BINDING if binding is None else binding
    if not (
        isinstance(binding, dict) and set(binding) == {"path", "raw_line_sha256", "semantic_subset"}
        and binding.get("path") == LOG_RELATIVE.as_posix()
        and isinstance(binding.get("raw_line_sha256"), str) and SHA256_RE.fullmatch(binding["raw_line_sha256"])
        and _exact(binding.get("semantic_subset"), RUN131_LOG_BINDING["semantic_subset"])
        and (not fixed or _exact(binding, RUN131_LOG_BINDING))
    ):
        raise ValueError("Run 131 log binding drifted")
    raw_log = _read_regular(root, LOG_RELATIVE, "append-only operational log", MAX_LOG_BYTES)
    lines = raw_log.splitlines(keepends=True)
    if len(lines) > MAX_LOG_LINES:
        raise ValueError("append-only operational log exceeds the line limit")
    ...
    for line in lines:
        ...
        matches.extend(hits)
    if len(matches) != 1:
        raise ValueError("Run 131 exact raw log line is absent or duplicated")
    record = _decode(matches[0].rstrip(b"\r\n"), "Run 131 log line")
    semantic = binding["semantic_subset"]
    if not isinstance(record, dict) or type(record.get("run")) is not int or type(record.get("metric")) is not int or any(
        record.get(name) != semantic[name] for name in ("run", "commit", "metric", "status", "description")
    ):
        raise ValueError("Run 131 log-line semantic subset drifted")
    controls: list[Any] = []
    def visit(value: Any) -> None:
        if isinstance(value, dict):
            if "observed_controls" in value:
                controls.append(value["observed_controls"])
            for child in value.values():
                visit(child)
        elif isinstance(value, list):
            for child in value:
                visit(child)
    visit(record)
    if len(controls) != 1 or not isinstance(controls[0], dict) or not _closed_control_tree(controls[0]):
        raise ValueError("Run 131 observed controls are not all closed")
    return binding


def _require_v4_active_intent_absent(root: Path) -> None:
    if os.path.lexists(root / V4_ACTIVE_INTENT_RELATIVE):
        raise ValueError("active v4 intent exists beside its consumed marker")
...
def _validate_pre_output_gates(
    ...
    if not (
        ...
        and gates.get("active_v4_launch_intent_must_be_absent") == V4_ACTIVE_INTENT_RELATIVE.as_posix()
        and (not fixed or _exact(gates, PRE_OUTPUT_GATES))
    ):
        raise ValueError("recovery-v5 pre-output gate schema drifted")
    legacy = _validate_legacy_pre_output_gates(
        root, {name: gates[name] for name in LEGACY_PRE_OUTPUT_GATES}
    )
    evidence = {
        "legacy_recovery_evidence": legacy,
        "consumed_v4_launch_intent": _validate_v4_consumed_intent(root, gates["consumed_v4_launch_intent"]),
        "v4_failure_tree": _validate_v4_failure(root, gates["actual_v4_failure_tree"]),
        "run_131_log_record": _validate_run131_log(root, gates["run_131_log_record"]),
    }
    _require_v4_active_intent_absent(root)
...
def _validate_receipt_static(
    ...
    plan_revision = _commit_has_bytes(
        root, plan.get("git_commit"), PLAN_RELATIVE, PLAN_RAW_SHA256, "recovery-v5 plan"
    )
    producer_revision = _commit_has_bytes(
        root, receipt.get("source_producer_git_commit"), PRODUCER_RELATIVE,
        PRODUCER_RAW_SHA256, "recovery-v5 producer",
    )
    if plan_revision != producer_revision:
        raise ValueError("receipt plan and producer commits differ")
    _require_ancestor(root, plan_revision, plan_head, "recovery-v5 plan")
    _require_ancestor(root, producer_revision, producer_head, "recovery-v5 producer")
...
def _validate_artifact_tree(root: Path, started: str, expected_files: set[str]) -> None:
    artifact = _directory_path(root, ARTIFACT_RELATIVE, "recovery-v5 output")
    try:
        with os.scandir(artifact) as entries:
            names = set()
            for entry in entries:
                if len(names) >= 3:
                    raise ValueError("recovery-v5 output inventory exceeds its bound")
                if entry.is_symlink():
                    raise ValueError("symlink in recovery-v5 output")
                names.add(entry.name)
    except OSError as error:
        raise ValueError("cannot inspect recovery-v5 output") from error
    if names != {RAW_DIRECTORY, "receipt.json", START_MARKER}:
        raise ValueError("recovery-v5 output inventory drifted")
...
def verify_artifact() -> tuple[int, str]:
    ...
    if not _exact(plan["pre_output_gates"], PRE_OUTPUT_GATES) or not _exact(
        ...
        raise ValueError("recovery-v5 plan pre-output bindings drifted")

    receipt = _decode(
        _read_regular(root, RECEIPT_RELATIVE, "recovery-v5 receipt", MAX_JSON_BYTES),
        "recovery-v5 receipt",
    )
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        consumed = root / V4_CONSUMED_INTENT_RELATIVE
        consumed.parent.mkdir(parents=True, exist_ok=True)
        consumed.write_bytes(consumed_raw)
        v4_directory = root / V4_FAILURE_RELATIVE
        (v4_directory / "raw_api_responses").mkdir(parents=True)
        v4_receipt_raw = _json_bytes(dict(V4_FAILURE_REQUIRED_FIELDS))
        ...
        gates = {
            ...
            "consumed_v4_launch_intent": {
                "path": V4_CONSUMED_INTENT_RELATIVE.as_posix(), "sha256": _sha256(consumed_raw),
            },
        ...
        assert V4_CONSUMED_INTENT_BINDING["sha256"] == "f625dbfdc61ac05b0825f89171331238636f9e5cd002cb3f6a6d0e9d0219895a"
        assert V4_FAILURE_BINDING["failure_receipt"]["sha256"] == "0c79b8564c804c93d49e07c0103b274829dfe5a21fef820ac7dcd11016b8f3d5"
        assert V4_FAILURE_BINDING["started_at_utc"]["sha256"] == "79246f3777035d2063217fec34be01b74479d9be7e337186dfbe7aff1ca71d5b"
        ...
        _reject(lambda: _validate_v4_failure(root, v4_binding), "v4 failure receipt hash")
        checks += 1
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
...
def main() -> int:
    ...
    if args.command == "self-test":
        checks = self_test()
        print("METRIC openmm86_deposited_ph_recovery_v5_checker_self_tests=%d" % checks)
        print("STATUS PASS")
        return 0
    ...
    try:
        ...
        print("REFUSAL fixed local recovery-v5 evidence is absent or invalid: %s" % error)
        print("STATUS HOLD_DEPOSITED_PH_METADATA_INCOMPLETE_OR_AMBIGUOUS")
        return 3
    print("METRIC openmm86_deposited_ph_recovery_v5_checker_checks=%d" % checks)
