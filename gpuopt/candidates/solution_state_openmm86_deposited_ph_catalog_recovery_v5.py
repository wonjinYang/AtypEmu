...
# Recovery-v5 binds the consumed v4 launch and its sealed producer failure in
# addition to every v1-v3 gate above.  The receipt raw digest is authoritative;
# required_fields make the fail-closed meaning explicit without normalizing it.
...
V4_CONSUMED_INTENT_SHA256 = (
    "f625dbfdc61ac05b0825f89171331238636f9e5cd002cb3f6a6d0e9d0219895a"
)
V4_FAILURE_RECEIPT_SHA256 = (
    "0c79b8564c804c93d49e07c0103b274829dfe5a21fef820ac7dcd11016b8f3d5"
)
V4_STARTED_AT_SHA256 = (
    "79246f3777035d2063217fec34be01b74479d9be7e337186dfbe7aff1ca71d5b"
)
...
V4_CONSUMED_INTENT_BINDING = {
    ...
    "sha256": V4_CONSUMED_INTENT_SHA256,
}
V4_FAILURE_BINDING = {
    ...
    "failure_receipt": {
        "path": "failure_receipt.json",
        "sha256": V4_FAILURE_RECEIPT_SHA256,
        "required_fields": V4_FAILURE_REQUIRED_FIELDS,
    },
    "started_at_utc": {
        "path": "started_at_utc.txt",
        "sha256": V4_STARTED_AT_SHA256,
    },
}
RUN131_LOG_BINDING = {
    "path": LOG_RELATIVE.as_posix(),
    "raw_line_sha256": RUN131_RAW_LINE_SHA256,
    "semantic_subset": {
        "run": 131,
        "commit": "fe15f37",
        "metric": 0,
...
def _v4_failure_directory(root: Path) -> Path:
    _safe_relative(V4_FAILURE_RELATIVE)
    directory = root
    for piece in V4_FAILURE_RELATIVE.parts:
        directory = directory / piece
        try:
            details = os.lstat(directory)
        except OSError as error:
            raise ValueError("missing sealed v4 failure directory") from error
        if stat.S_ISLNK(details.st_mode) or not stat.S_ISDIR(details.st_mode):
            raise ValueError("sealed v4 failure directory is indirect")
    return directory


def _read_v4_consumed_intent(
    root: Path, binding: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    ...
    if not (
        ...
        and isinstance(binding.get("sha256"), str)
        and re.fullmatch(r"[0-9a-f]{64}", binding["sha256"])
        and (not fixed or _exact_json(binding, V4_CONSUMED_INTENT_BINDING))
    ):
        raise ValueError("consumed v4 launch-intent binding drifted")
    raw = _read_regular(
        root, V4_CONSUMED_INTENT_RELATIVE, "consumed v4 launch intent", MAX_SOURCE_BYTES
    )
    if _sha256(raw) != binding["sha256"]:
...
def _read_v4_failure_evidence(
    ...
    if not (
        isinstance(binding, dict)
        and set(binding) == {
            "failure_directory", "failure_inventory", "failure_receipt", "started_at_utc"
        }
        and binding.get("failure_directory") == V4_FAILURE_RELATIVE.as_posix()
        and _required_json(binding.get("failure_receipt"), {
            "path": "failure_receipt.json",
            "required_fields": V4_FAILURE_REQUIRED_FIELDS,
        })
        and _required_json(binding.get("started_at_utc"), {"path": "started_at_utc.txt"})
        and (not fixed or _exact_json(binding, V4_FAILURE_BINDING))
    ):
    ...
    if not (
        isinstance(receipt_spec.get("sha256"), str)
        and re.fullmatch(r"[0-9a-f]{64}", receipt_spec["sha256"])
        and isinstance(marker_spec.get("sha256"), str)
        and re.fullmatch(r"[0-9a-f]{64}", marker_spec["sha256"])
    ):
    ...
    if not isinstance(inventory, dict):
        raise ValueError("sealed v4 failure inventory binding is malformed")
    expected_files = inventory.get("regular_files")
    expected_directories = inventory.get("directories")
    if expected_files != ["failure_receipt.json", "started_at_utc.txt"] or expected_directories != [
        {"path": "raw_api_responses", "inventory": []}
    ]:
    ...
    if files != set(expected_files) or directories != {"raw_api_responses"}:
    ...
    receipt_raw = _read_regular(
        root, V4_FAILURE_RELATIVE / "failure_receipt.json", "sealed v4 failure receipt", MAX_RESPONSE_BYTES
    )
    marker_raw = _read_regular(
        root, V4_FAILURE_RELATIVE / "started_at_utc.txt", "sealed v4 start marker", 1024
    )
    if _sha256(receipt_raw) != receipt_spec["sha256"]:
        raise ValueError("sealed v4 failure receipt raw SHA-256 drifted")
    if _sha256(marker_raw) != marker_spec["sha256"]:
        raise ValueError("sealed v4 start marker raw SHA-256 drifted")
    receipt = _loads(receipt_raw, "sealed v4 failure receipt")
    if not _required_json(receipt, receipt_spec["required_fields"]):
        raise ValueError("sealed v4 failure receipt fields drifted")
    return binding


def _read_run131_log_evidence(
    root: Path, binding: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    fixed = binding is None
    ...
    if not (
        ...
        and isinstance(binding.get("raw_line_sha256"), str)
        and re.fullmatch(r"[0-9a-f]{64}", binding["raw_line_sha256"])
        and _exact_json(binding.get("semantic_subset"), RUN131_LOG_BINDING["semantic_subset"])
        and (not fixed or _exact_json(binding, RUN131_LOG_BINDING))
    ):
    ...
    for line in lines:
        json_raw = line.rstrip(b"\r\n")
        candidates = [line] if line == json_raw else [line, json_raw]
        ...
        if hits:
            matches.append((hits[0], json_raw))
    if len(matches) != 1:
        raise ValueError("Run 131 exact raw log line is absent or duplicated")
    record = _loads(matches[0][1], "Run 131 log line")
    semantic = binding["semantic_subset"]
    if not isinstance(record, dict) or not all(
        record.get(name) == semantic[name]
        for name in ("run", "commit", "metric", "status", "description")
    ) or type(record.get("run")) is not int or type(record.get("metric")) is not int:
        raise ValueError("Run 131 log-line semantic subset drifted")
    control_nodes: List[Any] = []
    def visit(value: Any) -> None:
        if isinstance(value, dict):
            if "observed_controls" in value:
                control_nodes.append(value["observed_controls"])
            for child in value.values():
                visit(child)
        elif isinstance(value, list):
            for child in value:
                visit(child)
    visit(record)
    if len(control_nodes) != 1 or not isinstance(control_nodes[0], dict) or not _closed_control_tree(control_nodes[0]):
        raise ValueError("Run 131 observed controls are not all closed")
    return binding


def _require_v4_active_intent_absent(root: Path) -> None:
    path = root / V4_ACTIVE_INTENT_RELATIVE
    _safe_relative(V4_ACTIVE_INTENT_RELATIVE)
...
def _validate_pre_output_gates(
    ...
    """Validate all inherited gates plus exact consumed-v4 failure evidence."""
    fixed = gates is None
    gates = PRE_OUTPUT_GATES if gates is None else gates
    if not (
        isinstance(gates, dict)
        and set(gates) == set(PRE_OUTPUT_GATES)
    ...
    legacy_gates = {name: gates[name] for name in LEGACY_PRE_OUTPUT_GATES}
    legacy = _validate_legacy_pre_output_gates(root, legacy_gates)
    consumed = _read_v4_consumed_intent(root, gates["consumed_v4_launch_intent"])
    failure = _read_v4_failure_evidence(root, gates["actual_v4_failure_tree"])
    run131 = _read_run131_log_evidence(root, gates["run_131_log_record"])
    _require_v4_active_intent_absent(root)
    evidence = {
        "legacy_recovery_evidence": legacy,
        "consumed_v4_launch_intent": consumed,
        "v4_failure_tree": failure,
        "run_131_log_record": run131,
    }
    if fixed and not _exact_json(evidence, RECOVERY_EVIDENCE):
        raise ValueError("recovery-v5 pre-output recovery evidence drifted")
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        run122_raw = _run122_fixture_raw()
        run126_raw = _run126_fixture_raw()
        log = root / LOG_RELATIVE
        log.parent.mkdir(parents=True, exist_ok=True)
        run131_raw = json.dumps(
            {
        ...
        consumed_path = root / V4_CONSUMED_INTENT_RELATIVE
        ...
        v4_directory.mkdir(parents=True)
        (v4_directory / "raw_api_responses").mkdir()
        v4_receipt = dict(V4_FAILURE_REQUIRED_FIELDS)
        v4_receipt_raw = _json_bytes(v4_receipt)
        v4_marker_raw = b"2026-09-06T05:00:00Z\n"
        _write_new(v4_directory / "failure_receipt.json", v4_receipt_raw)
        _write_new(v4_directory / "started_at_utc.txt", v4_marker_raw)
        v4_binding = {
            "failure_directory": V4_FAILURE_RELATIVE.as_posix(),
            "failure_inventory": {
                "regular_files": ["failure_receipt.json", "started_at_utc.txt"],
        ...
        run131_binding = {
            "path": LOG_RELATIVE.as_posix(), "raw_line_sha256": _sha256(run131_raw),
        ...
        gates = {
            ...
            "run_122_log_record": {
                "path": LOG_RELATIVE.as_posix(),
                "raw_line_sha256": _sha256(run122_raw),
                "semantic_subset": RUN122_LOG_BINDING["semantic_subset"],
            },
            "run_126_log_record": {
                "path": LOG_RELATIVE.as_posix(),
                "raw_line_sha256": _sha256(run126_raw + b"\n"),
                "semantic_subset": RUN126_LOG_BINDING["semantic_subset"],
            },
            "v2_output_must_be_absent": V2_OUTPUT_RELATIVE.as_posix(),
            "v3_output_must_be_absent": V3_OUTPUT_RELATIVE.as_posix(),
        ...
        assert evidence["legacy_recovery_evidence"]["failed_v3_launch_intent"] == failed_v3_binding
        assert evidence["v4_failure_tree"] == v4_binding
        assert evidence["run_131_log_record"] == run131_binding
        assert V4_CONSUMED_INTENT_SHA256 == "f625dbfdc61ac05b0825f89171331238636f9e5cd002cb3f6a6d0e9d0219895a"
        assert V4_FAILURE_RECEIPT_SHA256 == "0c79b8564c804c93d49e07c0103b274829dfe5a21fef820ac7dcd11016b8f3d5"
        assert V4_STARTED_AT_SHA256 == "79246f3777035d2063217fec34be01b74479d9be7e337186dfbe7aff1ca71d5b"
        assert RUN131_RAW_LINE_SHA256 == "4e06e334dbb873b9c87e9df91c59df3d1c2cde1f623bb9b388d1f83694355451"
        assert not (root / OUTPUT_RELATIVE).exists()
        checks += 1
        os.unlink(v4_directory / "failure_receipt.json")
        _write_new(v4_directory / "failure_receipt.json", b"tampered")
        _expect_value_error(lambda: _read_v4_failure_evidence(root, v4_binding), "v4 failure receipt hash")
        checks += 1
    ...
    expected_url = "https://api.bmrb.io/v2/entry/42/Assigned_chem_shift_list?tag_list=x"
    _require_exact_official_url(expected_url, expected_url)
    _expect_value_error(
        lambda: _require_exact_official_url(
            "http://api.bmrb.io/v2/entry/42/Assigned_chem_shift_list?tag_list=x",
            expected_url,
        ),
    ...
    _expect_value_error(
        lambda: _require_exact_official_url(
            "https://api.bmrb.io:444/v2/entry/42/Assigned_chem_shift_list?tag_list=x",
            expected_url,
        ),
    ...
    _expect_value_error(
        lambda: _require_exact_official_url(
            "https://api.bmrb.io/v2/entry/42/Assigned_chem_shift_list?tag_list=y",
            expected_url,
        ),
...
def main() -> int:
    ...
    if args.command == "self-test":
        checks = self_test()
        print("METRIC openmm86_deposited_ph_recovery_v5_checks=%d" % checks)
        print("STATUS PASS")
        return 0
    ...
    try:
        ...
        print("STATUS CRASH_OPENMM86_DEPOSITED_PH_RECOVERY_V5")
        print("ERROR %s: %s" % (type(error).__name__, error))
        return 1
    print("METRIC openmm86_deposited_ph_recovery_v5_feasible_entities=%d/%d" % (
