#!/usr/bin/env python3
"""HOLD-only recovery-v2 for the recovery-v1 post-output implementation defect.

This separately named candidate changes exactly one operational detail: each
call into the frozen v1 fallback receives the three-loop mapping for that one
BMRB entry.  Metadata remains metadata only; it is not protonation, support,
science, score, target, or authorization evidence.
"""
...
RUN146_RAW_LINE_SHA256 = "184f3a4c600741ee2d56ea986d29d092c969f97a4b8b1a02192a3df9856ce86f"
...
# Its full output and Run 146 hashes are required to be syntactically complete,
# linked to the separately read bytes, and to carry the immutable supplied
# prefixes.  All other fields and the nested repair limit are exact.
...
def _validate_recovery_v1_source(root: Path) -> None:
    ...
    committed = _git(
        root,
        ("git", "show", "%s:%s" % (
            RECOVERY_V1_SOURCE_GIT_COMMIT, RECOVERY_V1_SOURCE_RELATIVE.as_posix()
        )),
    )
    if committed != raw or _sha256(committed) != RECOVERY_V1_SOURCE_SHA256:
        _fail("recovery-v1 source differs from its bound source commit")
...
def _read_run146_log(root: Path, expected_sha256: str) -> str:
    ...
    raw_log = _read_regular(root, RUN146_LOG_RELATIVE, "append-only operational log", MAX_LOG_BYTES)
    ...
    if len(matches) != 1:
        _fail("Run 146 exact raw log line is absent or duplicated")
    record = _loads(matches[0], "Run 146 log line")
    if not isinstance(record, dict) or type(record.get("run")) is not int or record.get("run") != 146:
        _fail("Run 146 log-line schema drifted")
    return expected_sha256


def _expected_plan(source_sha256: str) -> Dict[str, Any]:
    return {
        "artifact_kind": "hold_only_target_unread_unique_assigned_ph_recovery_v2_plan_not_authorization",
        "bound_failure": {
        ...
        "execution": {
            ...
            "external_execution_intent": RECOVERY_V2_INTENT_RELATIVE.as_posix(),
            "execute_flag": "--execute",
            "output_receipt": OUTPUT_RELATIVE.as_posix(),
            "receipt_creation": "O_EXCL; one separately named receipt only",
            "reviewed_binding": "A separately created exact-schema intent must bind committed Git HEAD, recovery-v2 source SHA-256, recovery-v2 plan SHA-256, both corrected archive SHA-256 values, and the three full terminal recovery-v1 failure SHA-256 values.",
        },
        ...
        "warnings": [
            "Recovery-v1 sealed output is terminal failure binding only and is not reused as evidence.",
...
def _validate_plan(plan: Any, source_sha256: str) -> None:
    if not _exact(plan, _expected_plan(source_sha256)):
        _fail("recovery-v2 plan schema, identities, or bindings drifted")
...
def _canonical_root() -> Path:
    ...
    if (
        ...
        _fail("recovery-v2 source path is indirect or misplaced")
    return root


def _validate_recovery_v2_intent(
    intent: Any, revision: str, source_sha256: str, plan_sha256: str,
) -> Dict[str, str]:
    expected = {
        "artifact_kind": "hold_only_unique_assigned_ph_recovery_v2_execution_intent",
        "candidate_id": CANDIDATE_ID,
    ...
    if not _exact(intent, expected):
        _fail("recovery-v2 execution intent schema or values drifted")
    return {
        "recovery_v1_postoutput_failure_receipt_raw_sha256": RECOVERY_V1_POSTOUTPUT_FAILURE_RECEIPT_SHA256,
        "recovery_v1_sealed_output_raw_sha256": RECOVERY_V1_OUTPUT_SHA256,
        "run_146_raw_line_sha256": RUN146_RAW_LINE_SHA256,
...
def _prearchive_validation(root: Path) -> Dict[str, str]:
    ...
    source_raw = _read_regular(root, CANDIDATE_RELATIVE, "recovery-v2 source", MAX_REGULAR_BYTES)
    source_sha256 = _sha256(source_raw)
    plan_raw = _read_regular(root, PLAN_RELATIVE, "recovery-v2 plan", MAX_REGULAR_BYTES)
    plan_sha256 = _sha256(plan_raw)
    _validate_plan(_loads(plan_raw, "recovery-v2 plan"), source_sha256)
    ...
    for relative, current, label in (
        (CANDIDATE_RELATIVE, source_raw, "recovery-v2 source"),
        (PLAN_RELATIVE, plan_raw, "recovery-v2 plan"),
    ):
    ...
    intent_raw = _read_regular(
        root, RECOVERY_V2_INTENT_RELATIVE, "recovery-v2 execution intent", MAX_REGULAR_BYTES
    )
    terminal_bindings = _validate_recovery_v2_intent(
        _loads(intent_raw, "recovery-v2 execution intent"), revision, source_sha256, plan_sha256
    )
    ...
    run146_sha256 = _read_run146_log(
        root, terminal_bindings["run_146_raw_line_sha256"]
    )
    ...
    _validate_postoutput_failure_receipt(
        ...
        sealed_output_sha256, run146_sha256,
    )
    return {
        "candidate_source_git_commit": revision,
        "candidate_source_raw_sha256": source_sha256,
        "execution_intent_path": RECOVERY_V2_INTENT_RELATIVE.as_posix(),
...
def _receipt(
    ...
    return {
        "artifact_kind": "hold_only_target_unread_unique_assigned_ph_recovery_v2_receipt",
        "bound_inputs": {
...
def _write_receipt_once(root: Path, payload: Dict[str, Any]) -> None:
    ...
    try:
        ...
        raise RecoveryError("recovery-v2 output already exists; no in-place retry") from error
    details = os.lstat(output)
    if stat.S_ISLNK(details.st_mode) or not stat.S_ISDIR(details.st_mode):
        _fail("new recovery-v2 output is indirect")
...
def self_test() -> int:
    ...
    bindings = _validate_recovery_v2_intent(
        recovery_v2_intent, "a" * 40, "b" * 64, "c" * 64
    )
    if bindings["recovery_v1_sealed_output_raw_sha256"] != RECOVERY_V1_OUTPUT_SHA256:
        raise AssertionError("recovery-v2 terminal binding extraction drifted")
    malformed_v2_intent = dict(recovery_v2_intent)
    malformed_v2_intent["run_146_raw_line_sha256"] = "0" * 64
    _expect_recovery_error(
        lambda: _validate_recovery_v2_intent(
            malformed_v2_intent, "a" * 40, "b" * 64, "c" * 64
        ),
        "recovery-v2 terminal SHA prefix",
    )
    checks += 3

    failure = dict(RECOVERY_V1_POSTOUTPUT_FAILURE_RECEIPT_FIXED)
    _validate_postoutput_failure_receipt(
        failure, RECOVERY_V1_OUTPUT_SHA256, RUN146_RAW_LINE_SHA256
    )
    failure_extra = dict(failure)
    failure_extra["unexpected"] = True
    _expect_recovery_error(
        lambda: _validate_postoutput_failure_receipt(
            failure_extra, RECOVERY_V1_OUTPUT_SHA256, RUN146_RAW_LINE_SHA256
        ),
    ...
    if CLOSED_CAPABILITIES != {key: False for key in CLOSED_CAPABILITIES}:
        raise AssertionError("recovery-v2 closed capability drift")
    checks += 1
    return checks
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.self_test or not (args.execute and args.acknowledge_hold_only):
        checks = self_test()
        print("METRIC openmm86_unique_assigned_ph_v1_recovery_v2_v1_semantic_checks=12")
        print("METRIC openmm86_unique_assigned_ph_v1_recovery_v2_synthetic_checks=%d" % checks)
    ...
    try:
        ...
        print("REFUSAL HOLD-only recovery-v2: %s" % error)
        return 3
