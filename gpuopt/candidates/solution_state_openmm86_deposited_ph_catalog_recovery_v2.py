...
def _read_v1_failure_evidence(root: Path) -> Dict[str, Any]:
    """Read exactly the sealed v1 failure before any recovery input or request."""
    ...
    if not stat.S_ISDIR(os.lstat(directory).st_mode):
        raise ValueError("sealed v1 failure path is not a directory")
    try:
    ...
    if inventory != set(V1_FAILURE_BINDING["failure_inventory"]):
...
def _load_plan(root: Path) -> Tuple[bytes, Dict[str, Any], str]:
    ...
    if not isinstance(plan, dict) or not (
        ...
        and plan.get("recovery_of") == V1_FAILURE_BINDING
    ):
...
def _failure_receipt(
    ...
    v1_failure_evidence: Dict[str, Any],
