...
def _validate_pre_output_gates(
    ...
    expected = RECOVERY_EVIDENCE if gates is PRE_OUTPUT_GATES else evidence
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        assert evidence["failed_v3_launch_intent"] == failed_v3_binding
...
def main() -> int:
    ...
    if args.command == "self-test":
        checks = self_test()
        print("METRIC openmm86_deposited_ph_recovery_v4_checks=%d" % checks)
        print("STATUS PASS")
        return 0
    ...
    try:
        ...
        print("STATUS CRASH_OPENMM86_DEPOSITED_PH_RECOVERY_V4")
        print("ERROR %s: %s" % (type(error).__name__, error))
        return 1
    print("METRIC openmm86_deposited_ph_recovery_v4_feasible_entities=%d/%d" % (
