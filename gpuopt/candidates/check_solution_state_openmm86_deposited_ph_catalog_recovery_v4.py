...
"""Independently replay the fixed local deposited-pH recovery-v4 output.
...
CHECKER_RELATIVE = Path(
    "gpuopt/candidates/check_solution_state_openmm86_deposited_ph_catalog_recovery_v4.py"
)
PLAN_RELATIVE = Path(
    "gpuopt/preunblind/"
    "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v4_plan.json"
)
...
ARTIFACT_RELATIVE = Path(
    ".auto/staging/atypemu_nested_support_count_v1_openmm86_deposited_ph_api_v4_recovery"
)
...
PLAN_CONTRACT = "atypemu_nested_support_count_v1_openmm86_deposited_ph_catalog_recovery_v4_plan"
API_BASE = "https://api.bmrb.io/v2"
APPLICATION_HEADER = "AtypEmu openmm86-deposited-ph-catalog-recovery-v4"
ENTITY_COUNT = 135
...
# Copied from the raw-byte-bound recovery-v4 plan.  These are fixed local
...
def _load_plan_and_producer(root: Path) -> tuple[dict[str, Any], str, str]:
    plan_head = _committed_exact(root, PLAN_RELATIVE, PLAN_RAW_SHA256, "recovery-v4 plan")
    producer_head = _committed_exact(
        root, PRODUCER_RELATIVE, PRODUCER_RAW_SHA256, "recovery-v4 producer"
    )
    plan = _decode(
        _read_regular(root, PLAN_RELATIVE, "recovery-v4 plan", MAX_SOURCE_BYTES),
        "recovery-v4 plan",
    )
    if not isinstance(plan, dict):
        raise ValueError("recovery-v4 plan root is not an object")
    ...
    if (
        ...
        raise ValueError("recovery-v4 plan semantic binding drifted")
...
def _validate_v1_failure(root: Path, binding: Any = V1_FAILURE_BINDING) -> dict[str, Any]:
    ...
    if not _exact(binding, V1_FAILURE_BINDING):
        raise ValueError("recovery-v4 v1 failure binding drifted")
...
def _validate_pre_output_gates(
    ...
    if not (
        ...
        raise ValueError("recovery-v4 pre-output gate schema or fixed paths drifted")
    evidence = {
    ...
    if fixed and not _exact(evidence, RECOVERY_EVIDENCE):
        raise ValueError("recovery-v4 pre-output evidence drifted")
...
def _validate_receipt_static(
    ...
    plan_revision = _commit_has_bytes(
        root, plan.get("git_commit"), PLAN_RELATIVE, PLAN_RAW_SHA256, "recovery-v4 plan"
    )
    producer_revision = _commit_has_bytes(
        root, receipt.get("source_producer_git_commit"), PRODUCER_RELATIVE,
        PRODUCER_RAW_SHA256, "recovery-v4 producer",
    )
    if plan_revision != producer_revision:
        raise ValueError("receipt plan and producer commits differ")
    _require_ancestor(root, plan_revision, plan_head, "recovery-v4 plan")
    _require_ancestor(root, producer_revision, producer_head, "recovery-v4 producer")
...
def _validate_artifact_tree(root: Path, started: str, expected_files: set[str]) -> None:
    artifact = _directory_path(root, ARTIFACT_RELATIVE, "recovery-v4 output")
    try:
        with os.scandir(artifact) as entries:
            names = set()
            for entry in entries:
                if len(names) >= 3:
                    raise ValueError("recovery-v4 output inventory exceeds its bound")
                if entry.is_symlink():
                    raise ValueError("symlink in recovery-v4 output")
                names.add(entry.name)
    except OSError as error:
        raise ValueError("cannot inspect recovery-v4 output") from error
    if names != {RAW_DIRECTORY, "receipt.json", START_MARKER}:
        raise ValueError("recovery-v4 output inventory drifted")
...
def verify_artifact() -> tuple[int, str]:
    ...
    if not _exact(plan["pre_output_gates"], PRE_OUTPUT_GATES) or not _exact(
        ...
        raise ValueError("recovery-v4 plan pre-output bindings drifted")

    receipt = _decode(
        _read_regular(root, RECEIPT_RELATIVE, "recovery-v4 receipt", MAX_JSON_BYTES),
        "recovery-v4 receipt",
    )
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        assert evidence["failed_v3_launch_intent"] == v3_binding
        assert evidence["run_126_log_record"] == gates["run_126_log_record"]
        checks += 1
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
...
def main() -> int:
    ...
    if args.command == "self-test":
        checks = self_test()
        print("METRIC openmm86_deposited_ph_recovery_v4_checker_self_tests=%d" % checks)
        print("STATUS PASS")
        return 0
    ...
    try:
        ...
        print("REFUSAL fixed local recovery-v4 evidence is absent or invalid: %s" % error)
        print("STATUS HOLD_DEPOSITED_PH_METADATA_INCOMPLETE_OR_AMBIGUOUS")
        return 3
    print("METRIC openmm86_deposited_ph_recovery_v4_checker_checks=%d" % checks)
