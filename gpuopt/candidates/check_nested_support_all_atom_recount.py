...
def _json_member(archive: tarfile.TarFile, name: str) -> tuple[dict[str, Any], bytes]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"non-object archive member: {name}")
    return value, raw
...
def _catalog_entities(raw: bytes) -> list[dict[str, Any]]:
    with tarfile.open(fileobj=io.BytesIO(raw), mode="r:gz") as archive:
        ...
        for index in range(27):
            shard, _ = _json_member(archive, f"./shard_{index}.json")
...
def verify(root: Path) -> int:
    ...
    with tarfile.open(fileobj=io.BytesIO(raw["result"]), mode="r:gz") as archive:
        members = archive.getmembers()
        ...
        summary, summary_raw = _json_member(archive, f"{PREFIX}/summary.json")
        if _sha256(summary_raw) != SHA256["summary"]:
        ...
        output_entities = []
        for index in range(27):
            shard, _ = _json_member(archive, f"{PREFIX}/shards/shard_{index}.json")
            ...
            for output in shard["entities"]:
                ...
                if (
                    ...
                    or accepted_indexes != sorted(accepted_indexes)
                    or rejected_indexes != sorted(rejected_indexes)
                    or set(accepted_indexes).intersection(rejected_indexes)
                    or set(accepted_indexes + rejected_indexes) != set(catalog_by_index)
                ):
